...
- **2026-09-09 リリース済み**。配布物は `dist_V16.2.1/`（2026-09-10 ビルド / commit 69bed8f）。
  ここに一時「未リリース」と書いたまま更新が漏れていた。後続の V16.2.2 の作業で
  「16.2.1 は未公開」という誤った前提を生んだので、現状を明示しておく。
- 2026-09-09 追記: **V16.2.0 に入れてしまった退行を1件、V16.2.1 で修正した**。ゴーストプレビューの
  プリミティブ用テクスチャで、Mirror Blend のぶん幅を 17→18 に広げたのに、行数を求める除算だけ
  旧値 68 のまま残っていた。**1本のツリーの要素が17個以上でプレビューが出なくなる**（16個までは
  偶然一致するため気づかない）。数えるのはレイアウト展開後なので、仕切りの配列を使っていれば
  少ない部品でも踏む。メッシュ生成は別経路のため無傷。**V16.2.0 は公開済みなので、購入者の環境で
  発生している**。詳細は「6-C. 公開後に見つかった退行」を参照。
...
### 6-C. 公開後に見つかった退行（V16.2.0 → V16.2.1 で修正）

V16.2.1 の公開準備中に、**V16.2.0 で入れてしまった退行**を含む複数の不具合を見つけて直した。
発見の経緯と再発防止をここに残す。

#### C-1. プレビュー用テクスチャの stride（最重要）

`handlers.py` の3箇所が揃っている必要がある。

1. テクスチャの幅（`PRIM_TEX_WIDTH`）
2. `_build_prim_data_for_element` が積む行数
3. `shader.py` の `texelFetch(primTex, ivec2(N, i), 0)` の最大 N（= 幅 - 1）

V16.1.3 までは 17 / 68 float / max index 16 で揃っていた。V16.2.0 で Mirror Blend の
`layer_p` 行を足した際、2 と 3 は 18 行 = 72 float に更新したが、
**`prim_count = len(prim_data) // 68` の除数だけ取り残された**。

`floor(72n / 68)` は n ≤ 16 では n に一致し、**n ≥ 17 で n を超える**。
その結果バッファ長 < テクスチャ必要長となり、Blender が
`ValueError: GPUTexture.__new__: Buffer size smaller than requested` を投げる。
描画ハンドラの中なので毎フレーム発生し、プレビューは丸ごと出なくなる。

実測（RTX 3060 / OpenGL、V16.2.0 の実コード）:

| 要素数 | バッファ長 | 算出行数 | 必要長 | 結果 |
|---:|---:|---:|---:|---|
| 16 | 1152 | 16 | 1152 | 成功 |
| 17 | 1224 | 18 | 1296 | 例外 |
| 20 | 1440 | 21 | 1512 | 例外 |

**数えるのはレイアウト展開後の要素数**。`_flatten_stack_for_preview` は仕切りの
`get_layout_matrices()` のぶん要素を複製するので、部品3個 + Grid 4×4 で18要素になる。
「17個も並べないから大丈夫」ではない。

再発防止として、幅と1要素あたりの float 数を定数化し（`PRIM_TEX_WIDTH` / `PRIM_TEX_FLOATS`）、
テクスチャ生成の直前に `_assert_prim_tex_stride()` で実データ長を検算する。
**ずれていても例外にせずプレビューを諦めるだけ**にしてあるので、次に誰かが行を足しても
メッシュ生成は止まらない。`tests_V16.2.1/test_preview_stride.py` が3箇所の一致を
20要素（17の壁を跨ぐ数）で検証する。
...
| 内容 | 場所 | 備考 |
|---|---|---|
| All Clear が非表示の出力を消し残す | `operators.py` | `select_set()` は hide_viewport のオブジェクトに効かない。`bpy.data.objects.remove` に変更 |
| All Clear が名前の部分一致で巻き込む | `operators.py` | `My_SDF_Backup` のような自作名まで削除していた。前方一致へ |
| `dc_last_error` がファイルに保存され蘇る | `__init__.py` | `is_gpu_ready` と同性質だが同期漏れ。engine は空文字を書き戻さないので自然回復しない |
| undo/redo からの強制デプスグラフ評価 | `handlers.py` | V16.1.2 でタイマー経路から外した危険パターンが残っていた。予防的措置 |
| `add_primitive` に `bl_options` が無い | `operators.py` | undo が中途半端な状態に戻る |
| タイマーの再描画が前面ウィンドウのみ | `engine.py` | 別ウィンドウの3Dビューが更新されない |
| `unregister` でタイマーが止まらない | `__init__.py` | ウォームアップ中に無効化すると回り続ける |
...
- **Linux は WSL2 で実検証できる**。Blender は公式 tarball を WSL 内に展開する。
  GPU は llvmpipe（ソフトウェア Vulkan）になるが、**メッシュの頂点数は Windows の実GPU と完全一致**
...
2026-09-11、外部からの指摘を起点に Mirror Blend の周辺を測り直し、プレビュー側の不具合を
1件修正した。Rust に触っていないため **V16.2.2** として切り出してある。
経緯・実測値・残作業は [HANDOVER_V16.2.2.md](HANDOVER_V16.2.2.md) を参照。
