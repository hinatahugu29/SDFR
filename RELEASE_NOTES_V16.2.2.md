# SDF.R V16.2.2 Release Notes — One Add-on, Separate Objects
...
*There is no V16.2.1. It was built and tested but never published: a fault was found in the Mirror
Blend preview before release, and fixing it produced this build instead. Everything V16.2.1 would
have contained is in V16.2.2.*
...
The **ghost preview approximates this**. The final mesh evaluates both sides and joins them, which
the preview cannot do in a single pass, so it estimates the same rounding instead. The two move
together and in the same direction — the preview swells as you raise Blend, exactly as the mesh does
— but they do not round identically. Measured on a mirrored sphere at offset 0.25, the preview sits
within 0.02–0.12 of the mesh across the useful range, widening to about 0.38 on two and three axes
at Blend 2.0.

**Blend past twice the Offset thickens the whole shape, not just the seam.** Once the blend radius
reaches across the gap between the two halves, there is no part of the shape far enough from the
seam to be left alone, and the surface inflates everywhere. This is inherent to joining the two
sides smoothly rather than a defect, and the panel warns you when your settings cross that line.
If you want a rounder seam without a heavier shape, raise Offset rather than Blend.

**Radial and Grid still crease.** They repeat space the same way, but blending across cell
boundaries needs neighbouring cells, which is a larger change than mirror's two sides. Mirror,
Radial and Grid on a **Collection Divider** have never had this problem, since those copy shapes
for real and join them with an ordinary smooth union.
