...
class SDF_DeformItem(bpy.types.PropertyGroup):
    ...
    deform_type: bpy.props.EnumProperty(
        items=[
            ('ELONGATE', "ストレッチ", "空間を引き伸ばす"),
            ('BEND', "ベンド", "空間を曲げる"),
            ('TWIST', "ツイスト", "空間をねじる"),
            ('TAPER', "テーパー", "空間を先細りにする"),
        ],
        name="種類", default='BEND', update=update_sdf_callback
    )
    axis: bpy.props.EnumProperty(
        items=[('0', "X", ""), ('1', "Y", ""), ('2', "Z", "")],
        name="軸", default='2', update=update_sdf_callback
    )
    factor: bpy.props.FloatProperty(name="係数", default=0.0, update=update_sdf_callback)
    origin: bpy.props.FloatVectorProperty(name="原点オフセット", default=(0.0, 0.0, 0.0), update=update_sdf_callback)
    # Elongate用（XYZ個別）
    ...
    enabled: bpy.props.BoolProperty(name="有効", default=True, update=update_sdf_callback)
...
class SDF_ObjectProperties(bpy.types.PropertyGroup):
    ...
    shape_type: bpy.props.EnumProperty(
        items=[
            ...
            ('mesh', "Mesh", "External Mesh Object")
        ],
    ...
    preview_quality: bpy.props.EnumProperty(
        items=[
            ('LOW', "低 (128)", "計算負荷を抑えます。シンプルな形状向け"),
            ('MID', "中 (256)", "バランスの良い設定。通常の変形向け"),
            ('HIGH', "高 (512)", "描画欠損を最小限に抑えます。激しい変形向け")
        ],
