...
class SDF_PT_main(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if not props.is_gpu_ready:
            ...
            col.separator()
            col.label(text="初期設定中（初回約2分ほどかかります）")
            col.label(text="Blenderを操作せずにお待ちください...")
            return
        ...
        if obj and getattr(obj, "sdf_props", None) and not obj.sdf_props.is_output:
            props = obj.sdf_props
            if props.is_primitive or (output_obj and output_obj.sdf_props.target_collection and obj.name in output_obj.sdf_props.target_collection.objects):
                box = layout.box()
                ...
                if props.blend_profile == '4':
                    row_blend.prop(props, "chamfer_smooth", text="係数")
                ...
                if props.edge_profile != '0':
                    row_edge.prop(props, "edge_profile_size", text="大きさ")
                
                if props.edge_profile == '4':
                    row_cs = sub.row(align=True)
                    row_cs.prop(props, "edge_chamfer_smooth", text="滑らかさ")
                
                sub.prop(props, "shell_thickness", text="Shell (中空化)")
                ...
                col_deform.label(text="変形 (スタック):", icon='MOD_SIMPLEDEFORM')
                ...
                if len(props.deform_stack) > 0 and props.deform_stack_index < len(props.deform_stack):
                    ...
                    sub.prop(d_item, "deform_type", text="種類")
                    sub.prop(d_item, "enabled", text="有効")
                    ...
                    if d_item.deform_type == 'ELONGATE':
                        ...
                        sub.prop(d_item, "factor", text="角度" if d_item.deform_type in ('BEND', 'TWIST') else "係数")
