...
pub struct SdfGpuContext {
    ...
    // V15.9.9.1: DCパイプラインコンパイル失敗時の診断文字列 (成功時はNone)
...
impl SdfGpuContext {
    ...
    /// V15.9.9.1: 直近のDCパイプラインコンパイル失敗の診断文字列を取得する
    ...
    pub async fn generate_mesh_gpu(&self, primitives: &[GpuPrimitive], bvh_nodes: &[GpuBvhNode], mut config: GpuConfig) -> (Vec<f32>, Vec<u32>) {
        ...
        // V15.9.9.2: 密グリッドマップ移行後、hash_keys はどのシェーダーからも参照されない死荷重の
        // ため、毎フレームのクリア(8MBコピー)を削除。block-ptr マップは hash_values のみ使用する。
    ...
    pub async fn generate_mesh_gpu_dc(&self, primitives: &[GpuPrimitive], bvh_nodes: &[GpuBvhNode], mut config: GpuConfig) -> (Vec<f32>, Vec<u32>) {
        ...
        let mut encoder = self.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: None });
        // V15.9.9.2: hash_keys は未使用の死荷重のため毎フレームクリア(8MBコピー)を削除 (generate_mesh_gpu と同様)。
        encoder.copy_buffer_to_buffer(&self.clear_values_buffer, 0, &self.hash_values_buffer, 0, (config.hash_table_size as u64 * 4));
        ...
        // V15.9.9.1: この時点ではまだ Detect Pass しか実行していないため、
        // 読み取れるのは active_count (counters[3]) のみ。v_count/i_count (counters[0]/[1]) は
        // Vertex Pass / Face Pass がまだ実行されておらず常に0のため、ここで判定してはいけない。
        // (元のコードはここで v_count/i_count も判定していたため、DCが常に空メッシュを
        //  返す原因になっていた。)
        ...
        // V15.9.9.1: Vertex/Face Pass 実行後に改めて counters[0]/[1] (v_count/i_count) を読み直す。
        encoder.copy_buffer_to_buffer(&self.counter_buffers[bb_idx], 0, &self.read_counter_buffer, 0, 32);
        self.queue.submit(Some(encoder.finish()));
