...
def update_sdf_mesh(output_obj, depsgraph=None):
    ...
    try:
        ...
        if backend == 'GPU_CHUNKED_DC' and hasattr(rust_gpu_sdf, "request_gpu_chunked_dc_update"):
            print(f"SDF Mesh Debug: requesting GPU chunked DC backend (Res: {res}, Chunk: {dc_chunk_cells})")
            ...
            print(f"SDF Mesh Debug: requesting GPU chunked MC backend (Res: {res}, Chunk: {chunk_cells})")
            ...
            print(f"SDF Mesh Debug: requesting CPU chunked MC backend (Res: {res}, Chunk: {chunk_cells})")
...
def sdf_mesh_timer():
    ...
    try:
        ...
        dc_err = rust_gpu_sdf.get_dc_last_error()
        if dc_err:
            print(f"SDF Mesh Debug: DC last error present: {dc_err}")
            scene = bpy.context.scene
            if hasattr(scene, "sdf_scene_props"):
