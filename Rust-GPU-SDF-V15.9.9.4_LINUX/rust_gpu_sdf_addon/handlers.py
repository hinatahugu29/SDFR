...
def draw_callback_3d(self, context):
    ...
    # --- V15.9.9.4: プリミティブデータが変化したときだけテクスチャを再構築 ---
    # 視点(カメラ)移動だけのフレームでは _preview_dirty が False のままなので、
    # スタック走査・行列decompose・64floatパック・GPUテクスチャ生成を丸ごと省略し、
    # キャッシュ済みテクスチャを再利用する。
    ...
    if interacting and max_steps > _INTERACTIVE_STEP_CAP:
        max_steps = _INTERACTIVE_STEP_CAP
