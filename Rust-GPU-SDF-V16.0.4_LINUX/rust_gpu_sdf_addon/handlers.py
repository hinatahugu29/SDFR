...
def _get_shape_extent(p_props):
    ...
    elif shape == 'gyroid': return max(p_props.p4, 0.1)
...
def _build_prim_data_for_element(el, o_props):
    ...
    if p_props and p_props.is_primitive:
        if p_props.shape_type == 'gyroid':
            data.extend([0.0, p_props.shell_thickness, float(int(p_props.gyroid_mask_shape)), float(int(p_props.gyroid_boundary_mode))])
        else:
