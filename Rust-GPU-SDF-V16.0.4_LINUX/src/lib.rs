...
fn sd_gyroid_field(p: Vector3<f32>, scale: f32, thickness: f32, bias: f32, extent: f32, mask_shape: f32, boundary_mode: f32, phase: f32, axis_scale_in: Vector3<f32>) -> f32 {
    ...
    let g = q.x.sin() * q.y.cos() + q.y.sin() * q.z.cos() + q.z.sin() * q.x.cos() + bias;
...
fn build_gpu_primitive_aabbs(primitives: &[SdfPrimitive], symmetry: u32) -> Vec<(Vector3<f32>, Vector3<f32>)> {
    ...
    for prim in primitives {
        ...
        let edge_pad = if prim.shape_type == "gyroid" { 0.0 } else { prim.edge_profile_size };
