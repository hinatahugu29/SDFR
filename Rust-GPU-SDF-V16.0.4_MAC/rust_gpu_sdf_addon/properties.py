...
class SDF_ObjectProperties(bpy.types.PropertyGroup):
    ...
    shape_type: bpy.props.EnumProperty(
        items=[
            ...
            ('gyroid', "Gyroid Field", "Periodic field surface clipped by extent")
        ],
    ...
    gyroid_preset: bpy.props.EnumProperty(
        items=[
            ('CUSTOM', "Custom", "Keep current Gyroid parameters"),
            ('FINE', "Fine", "Small cells with thin walls"),
            ('MEDIUM', "Medium", "Balanced Gyroid cells"),
            ('COARSE', "Coarse", "Large open cells"),
            ('THICK', "Thick", "Stronger thicker walls"),
            ('SHELL', "Shell", "Offset Gyroid shell pattern"),
        ],
        name="Gyroid Preset", default='MEDIUM', update=update_gyroid_preset
    )
    gyroid_boundary_mode: bpy.props.EnumProperty(
        ...
        name="Boundary", default='0', update=update_sdf_callback
    )
