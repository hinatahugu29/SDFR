...
fn sd_gyroid_field(p: vec3<f32>, scale: f32, thickness: f32, bias: f32, extent: f32, mask_shape: f32, boundary_mode: f32, phase: f32, axis_scale_in: vec3<f32>) -> f32 {
    ...
    let g = sin(q.x) * cos(q.y) + sin(q.y) * cos(q.z) + sin(q.z) * cos(q.x) + bias;
...
fn evaluate_shape(lp_in: vec3<f32>, prim: Primitive) -> f32 {
    ...
    else if (shape == 15u) { dp = sd_gyroid_field(lp_s, prim.extra_params.x, prim.extra_params.y, prim.extra_params.z, prim.extra_params.w, prim.modifier_params.z, prim.modifier_params.w, prim.gyroid_params.x, prim.gyroid_params.yzw); }
