...
fn detect_pass(@builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    var b_min = grid_to_world(vec3<f32>(wid * 8u)) - vec3<f32>(step * 2.5 + 0.1);
    ...
    if (all(lid == vec3<u32>(0u))) {
        ...
        for (var i = 0u; i < num_prims; i++) {
            ...
            if ((config.symmetry & 2u) != 0u) { center.y = abs(center.y); }
            if ((config.symmetry & 4u) != 0u) { center.z = abs(center.z); }
            let size = prim.size_and_op.xyz;
            let abs_size = abs(size);
            let prim_max_s = max(abs_size.x, max(abs_size.y, abs_size.z));
            ...
            if (shape_id == 0u) {
                ...
                let e = max(prim.extra_params.w, 0.01);
                if (prim.modifier_params.z >= 1.5) {
                    local_ext = vec3<f32>(e, e, e);
                } else if (prim.modifier_params.z >= 0.5) {
                    local_ext = vec3<f32>(e, e, e);
                } else {
                    local_ext = vec3<f32>(e, e, e);
                }
            ...
            if ((mode_flags & 2u) != 0u || (mode_flags & 4u) != 0u) {
                ...
                // V15.9.9.2: Spiral(bit2=4) は evaluate_layout で step*pitch (step=0..count-1) だけ
                // 軸方向にずれるため、螺旋全体は軸方向へ最大 (count-1)*pitch 伸びる。旧来は半径
                // (layout_data1.z) しか加算しておらず pitch(layout_data1.w) が完全に抜けていたため、
                // 背の高い螺旋の端インスタンスが未検出ブロックに落ちて丸ごと欠損していた。これを補う。
                // count は mode_flags(=layout_data1.x) の bit12..19 に格納 (evaluate_layout と同一)。
                if ((mode_flags & 4u) != 0u) {
                    let spiral_count = f32((mode_flags >> 12u) & 0xFFu);
                    bound_radius += abs(prim.layout_data1.w) * spiral_count;
                }
            }
            if ((mode_flags & 8u) != 0u) {
            ...
            if (packed_meta != 0u) {
                // V15.9.9.2: 変形あり prim の汎用セーフティを 0.75 -> 1.0 に微増。
                // F(グローバルCSG短絡撤去)により CSG シーンでもブロック毎カリングが効くようになり、
                // 変形 prim の bound 過小評価が面欠損に直結しやすくなったため、低コストで安全側へ倒す。
                bound_radius += prim_max_s * 1.0 + 0.25;
            }
    ...
    if (atomicLoad(&block_intersected) == 1u && all(lid == vec3<u32>(0u))) {
        ...
        // V15.9.9.2: 旧来はここで「シーンに CSG op が1つでもあれば (scene_has_csg_flag)、
        // 全ブロックを ALL_PRIMS 化してブロック毎カリングを完全放棄」していた。これは
        // subtract を1個置いただけでモデル全体のカリングが無効化され、全ブロックが全
        // プリミティブを評価する重大な悲観化になっていた。
        // 実際には下方 (has_csg_op ループ) に「そのブロックの候補リストが subtract/intersect
        // を含む場合のみ全リスト化」という per-block 判定が既に存在する。候補判定の
        // bound_radius は平滑度 k を含む (detect.wgsl 冒頭) ため CSG の非局所影響も局所化
        // されており、per-block 判定だけで correct-by-construction に絞り込める。
        // よってグローバル短絡を撤去し per-block 判定に委ねる。union のみの領域でカリングが
        // 復活し、boolean を含む複雑モデルで大幅に高速化する。
        // (scene_has_csg_flag への書き込みは無害なため残置。)

        if (raw_p_count > 64u) {
            block_prim_info[b_ptr].offset = 0u;
            block_prim_info[b_ptr].count = ALL_PRIMS_SENTINEL;
            active_blocks[b_ptr] = packed_id;
        insert_block_ptr(block_coord, i32(b_ptr));
            atomicAdd(&counters[4], 1u);
            return;
        }
