# V13: Primitive UI Definitions
# Defines display names and default values for each shape type

PRIMITIVE_UI_DEFS = {
    'sphere': {
        'label': "Sphere",
        'params': [
            ('radius', "Radius", 1.0),
        ]
    },
    'box': {
        'label': "Box",
        'params': [
            # Box uses standard scale(x,y,z), so p-params are unused
        ]
    },
    'rounded_box': {
        'label': "Rounded Box",
        'params': [
            ('p1', "Roundness", 0.1),
        ]
    },
    'torus': {
        'label': "Torus",
        'params': [
            ('p1', "Main Radius", 1.0),
            ('p2', "Pipe Radius", 0.3),
        ]
    },
    'cylinder': {
        'label': "Cylinder",
        'params': [
            ('p1', "Radius", 0.5),
            ('p2', "Height (Half)", 1.0),
        ]
    },
    'capsule': {
        'label': "Capsule",
        'params': [
            ('p1', "Radius", 0.3),
            ('p2', "Height (Half)", 0.7),
        ]
    },
    'hex_prism': {
        'label': "Hex Prism",
        'params': [
            ('p1', "Radius", 0.5),
            ('p2', "Height (Half)", 1.0),
        ]
    },
    'pyramid': {
        'label': "Pyramid",
        'params': [
            ('p1', "Base Size", 1.0),
            ('p2', "Height", 1.5),
        ]
    },
    'capped_cone': {
        'label': "Tapered Cylinder",
        'params': [
            ('p1', "Bottom Radius", 0.5),
            ('p2', "Top Radius", 0.3),
            ('p3', "Height", 1.5),
        ]
    },
    'ngon_prism': {
        'label': "N-gon Prism",
        'params': [
            ('p1', "Radius", 0.5),
            ('ngon_sides', "Sides", 6),
            ('p3', "Height", 1.0),
        ]
    },
    'ellipsoid': {
        'label': "Ellipsoid",
        'params': [
            ('p1', "Radius X", 1.0),
            ('p2', "Radius Y", 0.7),
            ('p3', "Radius Z", 0.5),
        ]
    },
    'rounded_cylinder': {
        'label': "Rounded Cylinder",
        'params': [
            ('p1', "Radius", 0.5),
            ('p2', "Edge Radius", 0.1),
            ('p3', "Height (Half)", 1.0),
        ]
    },
    'capped_torus': {
        'label': "Capped Torus",
        'params': [
            ('p1', "Main Radius", 1.0),
            ('p2', "Pipe Radius", 0.3),
            ('p3', "Angle (rad)", 2.0),
        ]
    },
    'octahedron': {
        'label': "Octahedron",
        'params': [
            ('p1', "Size", 1.0),
        ]
    },
    'cut_sphere': {
        'label': "Cut Sphere",
        'params': [
            ('p1', "Cut Height", 0.0),
        ]
    },
    'math_field': {
        'label': "Math Field",
        'params': [
            ('p1', "Scale", 5.0),
            ('p2', "Thickness", 0.08),
            ('p3', "Bias", 0.0),
            ('p4', "Extent", 1.6),
        ]
    },
    'extrude': {
        'label': "Extrude (2D Profile)",
        # Depth / Corner Radius は ui.py の専用ブロックで表示するのでここには含めない
        # （含めると二重表示になる）。初期値の適用にはこの params リストが使われる。
        'params': [
            ('p1', "Size X", 1.0),
            ('p2', "Size Y", 1.0),
        ]
    },
    'lathe': {
        'label': "Lathe (Revolve)",
        # Offset / Corner Radius は ui.py の専用ブロックで表示するのでここには含めない
        'params': [
            ('p1', "Size X", 1.0),
            ('p2', "Size Y", 1.0),
        ]
    },
    'bezier_curve': {
        'label': "Bezier Curve (3D)",
        # Control Points / Start-End Radius は ui.py の専用ブロックで表示するのでここには含めない
        'params': []
    },
}

# --- Inherited constants from V12.1 ---
_PRIM_COLORS = [
    (0.2, 0.5, 1.0, 1.0), (1.0, 0.3, 0.3, 1.0), (0.3, 0.9, 0.4, 1.0), (1.0, 0.7, 0.1, 1.0),
    (0.8, 0.3, 1.0, 1.0), (0.1, 0.9, 0.9, 1.0), (1.0, 0.5, 0.7, 1.0), (0.6, 0.8, 0.2, 1.0),
]

_fsq_coords = [(-1, -1, 0), (1, -1, 0), (1, 1, 0), (-1, 1, 0)]
_fsq_indices = [(0, 1, 2), (0, 2, 3)]

_SHAPE_MAP = {
    'sphere': 0.0,
    'box': 1.0,
    'torus': 2.0,
    'cylinder': 3.0,
    'rounded_box': 4.0,
    'capsule': 5.0,
    'hex_prism': 6.0,
    'pyramid': 7.0,
    'capped_cone': 8.0,
    'ngon_prism': 9.0,
    'ellipsoid': 10.0,
    'rounded_cylinder': 11.0,
    'capped_torus': 12.0,
    'octahedron': 13.0,
    'cut_sphere': 14.0,
    'math_field': 15.0,
    'extrude': 16.0,
    'lathe': 17.0,
    'bezier_curve': 18.0,
}

PROFILE_2D_INDEX = {
    'box': 0,
    'circle': 1,
    'ngon': 2,
    'star': 3,
}

# --- Math Field (数理場) ---
# shape_type='math_field' はこの1種類のみで、実際に使う数式は field_type プロパティで選ぶ。
# 新しい式を足す場合は、ここ(FIELD_TYPE_INDEX)とlib.rs/common.wgsl/shader.pyのtpms_formulaに
# 1エントリずつ追加するだけでよい。
FIELD_TYPE_INDEX = {
    'gyroid': 0,
    'schwarz_p': 1,
    'schwarz_d': 2,
}
