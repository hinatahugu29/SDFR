...
def _no_depsgraph_retry_exhausted(result):
    ...
    """
    global _pending_update, _pending_retry_count
    if result != "no_depsgraph":
        _pending_retry_count = 0
        return False

    _pending_retry_count += 1
    if _pending_retry_count < _PENDING_RETRY_LIMIT:
        return False

    print("SDF Info: depsgraph was unavailable for "
          f"{_pending_retry_count} consecutive attempts; dropping the queued update.")
    _pending_update = False
    _pending_retry_count = 0
    return True
            ...
            if _pending_update:
                for obj in bpy.context.scene.objects:
                    if obj.sdf_props.is_output:
                        ...
                        if _no_depsgraph_retry_exhausted(result):
                            _is_timer_registered = False
                            return None
