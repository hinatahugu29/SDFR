...
# 非同期更新管理用 (V10.3)
#
# V16.2.1: 出力オブジェクトごとに分離した。
# 以前はすべて単一のグローバルだったため、出力（SDFツリー）が2本以上あると
# 別のツリーの予約・リトライ状態を互いに踏み合った。Rust 側は GPU_CONTEXT が
# 単一 Mutex なので計算自体は常に1本ずつだが、「いま走っている計算が誰のものか」
# は Python 側で覚えておく必要がある（_inflight_owner）。
_pending_updates = {}          # {output_name: True} 更新の予約
# V16.1.2: 予約された更新が投げられないまま空回りするのを防ぐカウンタ。
# 0.05秒間隔なので 40 回でおよそ 2 秒。
_pending_retry_counts = {}     # {output_name: int}
_PENDING_RETRY_LIMIT = 40
_is_timer_registered = False
_force_next_normals = False
_last_mesh_requests = {}       # {output_name: (primitives, res, domain, use_dc, sym_mask, weld)}
_safe_retry_active = {}        # {output_name: bool}
_gpu_chunked_mc_active = {}    # {output_name: bool}
_gpu_chunked_dc_active = {}    # {output_name: bool}
_cpu_chunked_fallback_active = {}  # {output_name: bool}
# いま Rust 側で走っている（もしくは結果待ちの）計算の持ち主。
# fetch_mesh_if_ready() は「誰の結果か」を返さないため、ここで対応付ける。
_inflight_owner = None
_in_update = False
...
_last_state_hashes = {}
_cached_divider_names_by_output = {}


def is_cached_divider(name):
    """名前が、いずれかのツリーの仕切りEmptyかどうか。

    V16.2.1: 以前は単一の集合 `_cached_divider_names` を直接参照していた。
    """
    for names in _cached_divider_names_by_output.values():
        if name in names:
            return True
    return False
...
def get_sdf_state_fingerprint(output_obj, depsgraph):
    ...
    state = [
        props.resolution,
        props.domain_size,
        # V16.2.1: auto_domain / use_live_normals が抜けていた。どちらも
        # update=update_sdf_callback 付きなので update_sdf_mesh は呼ばれるが、
        # ハッシュが変わらないため「変更なし」と判定されて何も起きなかった
        # （Auto Expand Domain のトグルが効かない原因）。
        props.auto_domain,
        props.use_live_normals,
        props.algo_type,
...
def sync_sdf_stack(output_obj):
    ...
    # V16.2.1: 仕切りEmpty名は出力オブジェクトごとに持つ。単一の集合に入れていたため、
    # ツリーが2本以上あると最後に同期したツリーの分で上書きされ、それ以外のツリーの
    # 仕切りを動かしても depsgraph ハンドラが「関係あり」と判定できなかった。
    _cached_divider_names_by_output[output_obj.name] = {
        item.empty_ptr.name for item in props.sdf_stack
        if item.item_type == 'COLLECTION' and item.empty_ptr
    }
...
def _no_depsgraph_retry_exhausted(result, owner):
    """デプスグラフが取れないまま再試行が続いていないか判定し、諦めるべきなら True。

    数える対象は update_sdf_mesh が "no_depsgraph" を返した場合**だけ**に限る。

    update_sdf_mesh には「ドラッグ操作中」「Rust が計算中」「状態が変わっていない」
    といった理由で何もせず戻る経路が元からあり、これらは正常な待機なので数えない。
    実際、ドラッグを数秒続けただけで発動してしまう誤検知を出したことがある。

    数えるのは、評価を強制しない経路で view_layer.depsgraph がまだ用意されていない
    ケースのみ。これは通常すぐ解消するので、上限まで続くなら何かがおかしい。
    諦めても、次にユーザーが操作すれば depsgraph ハンドラから更新が走る。

    V16.2.1: カウンタは出力オブジェクトごとに持つ。以前は単一グローバルだったため、
    片方のツリーが正常に更新されるたびに、もう片方のカウンタまでリセットされていた。
    """
    if result != "no_depsgraph":
        _pending_retry_counts.pop(owner, None)
        return False

    count = _pending_retry_counts.get(owner, 0) + 1
    _pending_retry_counts[owner] = count
    if count < _PENDING_RETRY_LIMIT:
        return False

    print("SDF Info: depsgraph was unavailable for "
          f"{count} consecutive attempts; dropping the queued update for '{owner}'.")
    _pending_updates.pop(owner, None)
    _pending_retry_counts.pop(owner, None)
    return True


def _acquire_depsgraph(allow_forced_eval):
    """デプスグラフを取得する。取れなければ None。

    V16.1.2: `bpy.context.evaluated_depsgraph_get()` は、必要と判断すれば
    **その場でシーン全体の再評価を走らせる**。オペレーターやプロパティ更新の中でなら
    Blender が想定している流れなので問題ない。

    しかし `bpy.app.timers` のコールバックから呼ぶと、Blender が評価を予定していない
    タイミング（wm_event_do_notifiers）で評価が始まる。シーンの構成が直前に変わって
    いると、評価側が古い個数のまま Base（オブジェクトのビューレイヤ登録）を引き、
    範囲外アクセスでクラッシュする。

      py_timer_execute → evaluated_depsgraph_get → scene_graph_update_tagged
        → deg_evaluate_on_refresh → BKE_object_eval_eval_base_flags → SIGSEGV

    macOS / Blender 5.2 でプリミティブ追加時に落ちる報告があり、クラッシュログの
    バックトレースがこの経路そのものだった。Factory Settings では再現せず、
    多数のアドオンを有効にした環境でのみ再現する（他アドオンがシーンを触ることで
    危険な状態に入りやすくなる）。

    そのためタイマー経由では allow_forced_eval=False で呼び、評価を強制しない
    `view_layer.depsgraph` を使う。まだ用意されていなければ None を返し、
    呼び出し側は今回の更新を見送る（タイマーが再試行する）。
    """
    if allow_forced_eval:
        return bpy.context.evaluated_depsgraph_get()
    try:
        return getattr(bpy.context.view_layer, "depsgraph", None)
    except Exception:
        return None
...
def update_sdf_mesh(output_obj, depsgraph=None, allow_forced_eval=True):
    ...
    try:
        ...
        _gpu_chunked_dc_active[owner] = False
        ...
        if backend == 'GPU_CHUNKED_DC' and hasattr(rust_gpu_sdf, "request_gpu_chunked_dc_update"):
            ...
            _gpu_chunked_dc_active[owner] = requested
            ...
            _gpu_chunked_mc_active[owner] = requested
            ...
            _cpu_chunked_fallback_active[owner] = requested
        else:
            requested = rust_gpu_sdf.request_mesh_update(primitives, res, domain, use_dc, sym_mask, w_thresh)

        if requested:
            # V16.2.1: 受理されたのでここで初めてハッシュを確定し、この計算の
            # 持ち主を記録する。fetch_mesh_if_ready() は誰の結果かを返さないため、
            # タイマーはこの名前を頼りに結果を1体だけへ適用する。
            _last_state_hashes[output_obj.name] = current_hash
            _inflight_owner = owner
            _pending_updates.pop(owner, None)
            # タイマーがなければ登録
            if not _is_timer_registered:
                bpy.app.timers.register(sdf_mesh_timer, first_interval=0.05)
            ...
            # 要求が拒否された（Rust が計算中）。ハッシュは進めず、予約して次を待つ。
            _pending_updates[owner] = True
            if not _is_timer_registered:
                bpy.app.timers.register(sdf_mesh_timer, first_interval=0.05)
                _is_timer_registered = True
...
def apply_mesh_data(output_obj, data, force_normals=False):
    """計算済みデータをメッシュに適用する"""
    global _inflight_owner
    # V16.2.1: リトライ状態は出力オブジェクトごと。リトライ要求を出した場合は
    # 引き続きこの出力が持ち主になる。
    owner = output_obj.name
    last_request = _last_mesh_requests.get(owner)
    try:
        diagnostics = ""
        scene = bpy.context.scene
        ...
        if output_obj.sdf_props.protect_partial_mesh and health in {"CAPACITY_LIMITED", "EMPTY_RESULT"}:
            props = output_obj.sdf_props
            if props.auto_gpu_chunked_fallback and not _gpu_chunked_mc_active.get(owner) and last_request and hasattr(rust_gpu_sdf, "request_gpu_chunked_mesh_update"):
                primitives, prev_res, domain, _use_dc, sym_mask, w_thresh = last_request
                ...
                if rust_gpu_sdf.request_gpu_chunked_mesh_update(primitives, prev_res, domain, sym_mask, chunk_w_thresh, chunk_cells):
                    _gpu_chunked_mc_active[owner] = True
                    _inflight_owner = owner
                    _pending_updates.pop(owner, None)
                    return "retry"
            if props.auto_safe_retry and not _safe_retry_active.get(owner) and last_request:
                primitives, prev_res, domain, use_dc, sym_mask, w_thresh = last_request
                ...
                if retry_res < prev_res:
                    ...
                    if rust_gpu_sdf.request_mesh_update(primitives, retry_res, domain, use_dc, sym_mask, w_thresh):
                        _safe_retry_active[owner] = True
                        _inflight_owner = owner
                        _pending_updates.pop(owner, None)
                        return "retry"
            if props.auto_chunked_fallback and not _cpu_chunked_fallback_active.get(owner) and last_request and hasattr(rust_gpu_sdf, "request_chunked_mesh_update"):
                primitives, prev_res, domain, _use_dc, sym_mask, w_thresh = last_request
                chunk_cells = _choose_chunk_cells(prev_res, props.chunked_fallback_cells)
                chunk_w_thresh = _chunk_weld_threshold(props, w_thresh)
                ...
                if rust_gpu_sdf.request_chunked_mesh_update(primitives, prev_res, domain, sym_mask, chunk_w_thresh, chunk_cells):
                    _cpu_chunked_fallback_active[owner] = True
                    _inflight_owner = owner
                    _pending_updates.pop(owner, None)
                    return "retry"
        ...
        # V16.1.1: インデックスが頂点範囲に収まっているかを検証する。
        #
        # mesh.loops.foreach_set("vertex_index", ...) は値の妥当性を検査しない。
        # 範囲外のインデックスをそのまま書き込むと、Blender は存在しない頂点を指す
        # ループを持ったまま評価に入り、確保していないメモリを読む。多くの場合その場
        # では落ちず、あとから無関係な処理（デプスグラフ評価など）でクラッシュする
        # ため、原因の特定が極めて困難になる。
        # NaN/Inf を潰しているのと同じ理由で、ここで物理的に食い止める。
        if len(indices_np) > 0:
            v_total = len(verts_np)
            idx_min = int(indices_np.min())
            idx_max = int(indices_np.max())
            if idx_min < 0 or idx_max >= v_total:
                print(
                    "SDF Mesh ERROR: vertex index out of range "
                    f"(min={idx_min}, max={idx_max}, vertex count={v_total}). "
                    "Mesh rejected to avoid memory corruption."
                )
                return "error"
...
def _issue_pending_requests():
    """予約されている更新を投げる。1つでも投げられたら True。

    V16.2.1: 以前はループ内で無条件に return していたため、2本目以降の出力の
    予約が永久に処理されなかった。Rust 側は1本ずつしか受け付けないので、
    受理されなかったものは予約に残り、次のタイマーで再度試される。
    """
    global _is_timer_registered
    issued = False
    for name in list(_pending_updates.keys()):
        obj = _find_output_by_name(name)
        if obj is None:
            # 出力オブジェクトが消えた（削除・Finalize 済み）
            _pending_updates.pop(name, None)
            _pending_retry_counts.pop(name, None)
            continue
        # タイマーからはデプスグラフの評価を強制しない（_acquire_depsgraph 参照）
        result = update_sdf_mesh(obj, allow_forced_eval=False)
        if _no_depsgraph_retry_exhausted(result, name):
            continue
        issued = True
        if rust_gpu_sdf.is_updating():
            # GPU は1本ずつ。残りは次のタイマーで投げる。
            break
    return issued
...
def sdf_mesh_timer():
    ...
    try:
        ...
        if data:
            # V16.2.1: 結果は「その計算を要求した出力」にだけ適用する。
            # 以前は全出力に同じデータを流し込んでいたため、ツリーが2本以上あると
            # 全部が同じ形になっていた。fetch_mesh_if_ready() は持ち主を返さないので、
            # 要求時に記録した _inflight_owner を使う。
            owner_obj = _find_output_by_name(_inflight_owner)
            if owner_obj is None:
                # 持ち主が分からない場合（アドオン再読み込み直後など）は、
                # 出力が1本だけならそれに適用する。複数ある場合は捨てる方が安全。
        ...
        # 結果が来ないまま計算も止まっているので、投げっぱなしの計算は無い
        _inflight_owner = None
