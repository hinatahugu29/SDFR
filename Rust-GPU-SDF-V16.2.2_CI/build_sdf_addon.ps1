...
# `py -3.11` が第一候補。GitHub Actions の windows ランナーでは setup-python が入れた
# 3.11 を py ランチャーが知らないことがあるので、その場合は PATH の python を使う。
$pythonExe311 = $null
try {
    $probe = & py -3.11 -c "import sys; print(sys.prefix)" 2>$null
    if ($LASTEXITCODE -eq 0 -and $probe) { $pythonExe311 = "py"; $pythonPath = $probe }
} catch { }
if (-not $pythonExe311) {
    $ver = & python -c "import sys; print('%d.%d' % sys.version_info[:2])" 2>$null
    if ($LASTEXITCODE -ne 0) { throw "Python 3.11 が見つかりません" }
    if ($ver -ne "3.11") { throw "Python 3.11 が必要ですが python は $ver でした" }
    $pythonExe311 = "python"
    $pythonPath = & python -c "import sys; print(sys.prefix)"
}
Write-Host "Using Python: $pythonExe311 ($pythonPath)"
...
if ($pythonExe311 -eq "py") { & py -3.11 $zipScriptPath } else { & python $zipScriptPath }
...
if ($pythonExe311 -eq "py") { & py -3.11 $validateScriptPath } else { & python $validateScriptPath }
