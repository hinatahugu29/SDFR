...
use gpu::{SdfGpuContext, GpuConfig, GpuBvhNode};
...
static LAST_MESH_DIAGNOSTICS: Lazy<Mutex<String>> = Lazy::new(|| Mutex::new(String::from("health=NO_MESH; algo=none")));
...
pub struct SdfPrimitive {
    ...
    // V16.2.1: Layout Mirror の折り返し面をどれだけ丸めるか。
    // 0 なら従来どおり（ハードな折り返し＝折り目が出る）。
    #[pyo3(get, set)]
    pub mirror_blend: f32,
...
fn sd_gyroid_mask(p: Vector3<f32>, extent: f32, mask_shape: f32) -> f32 {
    let e = extent.max(0.01f32);
    if mask_shape >= 1.5f32 {
        let d = Vector2::new(Vector2::new(p.x, p.y).norm() - e, p.z.abs() - e);
        return d.x.max(0.0).hypot(d.y.max(0.0)) + d.x.max(d.y).min(0.0);
    }
    if mask_shape >= 0.5f32 {
        return p.norm() - e;
    }
    let b = p.abs() - Vector3::new(e, e, e);
    b.x.max(0.0).hypot(b.y.max(0.0).hypot(b.z.max(0.0))) + b.x.max(b.y.max(b.z)).min(0.0)
}
...
fn tpms_formula(field_type: u32, q: Vector3<f32>, bias: f32) -> f32 {
    match field_type {
        1 => q.x.cos() + q.y.cos() + q.z.cos() + bias, // Schwarz P
        2 => {
            q.x.sin() * q.y.sin() * q.z.sin()
                + q.x.sin() * q.y.cos() * q.z.cos()
                + q.x.cos() * q.y.sin() * q.z.cos()
                + q.x.cos() * q.y.cos() * q.z.sin()
                + bias
        } // Schwarz D (Diamond)
        _ => q.x.sin() * q.y.cos() + q.y.sin() * q.z.cos() + q.z.sin() * q.x.cos() + bias, // Gyroid
    }
}

fn sd_tpms_field(field_type: u32, p: Vector3<f32>, scale: f32, thickness: f32, bias: f32, extent: f32, mask_shape: f32, boundary_mode: f32, phase: f32, axis_scale_in: Vector3<f32>) -> f32 {
    let freq = scale.max(0.001f32);
    let thick = thickness.max(0.001f32);
    let axis_scale = Vector3::new(
        axis_scale_in.x.max(0.05f32),
        axis_scale_in.y.max(0.05f32),
        axis_scale_in.z.max(0.05f32),
    );
    let q = p.component_mul(&axis_scale) * freq + Vector3::new(phase, phase, phase);
    let g = tpms_formula(field_type, q, bias);
    let field = g.abs() / freq - thick;
    let e = extent.max(0.01f32);
    let mask_d = sd_gyroid_mask(p, extent, mask_shape);
    let wall = -mask_d;
    if boundary_mode >= 1.5f32 {
        return field.max(mask_d);
    }
    if wall < 0.0 {
        return -wall + thick;
    }
    if boundary_mode >= 0.5f32 {
        return field;
    }
    let fade_width = (0.9f32 / freq).clamp(0.08f32, e * 0.45f32);
    let t = (wall / fade_width).clamp(0.0f32, 1.0f32);
    let fade = t * t * (3.0f32 - 2.0f32 * t);
    field + (1.0f32 - fade) * (thick * 2.0f32 + 0.02f32)
}

/// 2Dプロファイル評価（common.wgsl の eval_2d_profile と同じ挙動）
/// prof_type: 0=Rectangle/Box, 1=Circle
fn eval_2d_profile(prof_type: u32, p: Vector2<f32>, size: Vector2<f32>, corner_r: f32) -> f32 {
    if prof_type == 1 {
        return p.norm() - size.x.max(0.001f32);
    }
    let cr = corner_r.clamp(0.0f32, size.x.min(size.y));
    let b = size - Vector2::new(cr, cr);
    let d = Vector2::new(p.x.abs(), p.y.abs()) - b;
    d.x.max(0.0).hypot(d.y.max(0.0)) + d.x.max(d.y).min(0.0) - cr
}

/// テーパー付き3D二次ベジェのSDF（common.wgsl の sd_bezier_eval と同じ挙動）
fn sd_bezier_taper(
    pos: Vector3<f32>,
    a_pt: Vector3<f32>,
    b_pt: Vector3<f32>,
    ...
    let a = b_pt - a_pt;
    let b = a_pt - b_pt * 2.0 + c_pt;
    let c = a * 2.0;
    let d = a_pt - pos;

    let kk = 1.0 / b.dot(&b).max(0.00001f32);
    let kx = kk * a.dot(&b);
    let ky = kk * (2.0 * a.dot(&a) + d.dot(&b)) / 3.0;
    let kz = kk * d.dot(&a);

    let p_val = ky - kx * kx;
    let p3 = p_val * p_val * p_val;
    let q_val = kx * (2.0 * kx * kx - 3.0 * ky) + kz;
    let h = q_val * q_val + 4.0 * p3;

    let res_t = if h >= 0.0 {
        let h_sqrt = h.sqrt();
        let x1 = (-q_val + h_sqrt) * 0.5;
        let x2 = (-q_val - h_sqrt) * 0.5;
        let u = x1.signum() * x1.abs().powf(1.0 / 3.0);
        let v = x2.signum() * x2.abs().powf(1.0 / 3.0);
        (u + v - kx).clamp(0.0, 1.0)
    } else {
        let z = (-p_val).sqrt();
        let v = (q_val / (p_val * z * 2.0)).clamp(-1.0, 1.0).acos() / 3.0;
        let m = v.cos();
        let n = v.sin() * 1.732050808f32;
        let t1 = ((m + m) * z - kx).clamp(0.0, 1.0);
        let t2 = ((-m - n) * z - kx).clamp(0.0, 1.0);
        let pos1 = a_pt + (c + b * t1) * t1 - pos;
        let pos2 = a_pt + (c + b * t2) * t2 - pos;
        if pos1.dot(&pos1) < pos2.dot(&pos2) { t1 } else { t2 }
    };

    let pos_on_curve = a_pt + (c + b * res_t) * res_t - pos;
    let radius_t = r_start * (1.0 - res_t) + r_end * res_t;
    pos_on_curve.norm() - radius_t
}
...
fn calculate_primitive_sdf(local_p: Vector3<f32>, primitive: &SdfPrimitive, bvh: &Option<MeshBvh>) -> f32 {
    ...
    let d = match primitive.shape_type.as_str() {
        ...
        "math_field" => {
            ...
            // 数式選択(field_type: 0=Gyroid, 1=Schwarz P, 2=Schwarz D)を運ぶスロットとして流用する
            sd_tpms_field(
                primitive.edge_profile,
                p,
                primitive.extra_params[0],
                primitive.extra_params[1],
                primitive.extra_params[2],
                primitive.extra_params[3],
                primitive.edge_chamfer_smooth,
                primitive.edge_profile_size,
                primitive.gyroid_params[0],
                Vector3::new(primitive.gyroid_params[1], primitive.gyroid_params[2], primitive.gyroid_params[3]),
            )
        }
        // Extrude / Lathe: edge_profile が 2Dプロファイル種類(prof_type)を運ぶ
        "extrude" => {
            let size_xy = Vector2::new(primitive.extra_params[0], primitive.extra_params[1]);
            let depth = primitive.extra_params[2];
            let corner_r = primitive.extra_params[3];
            let chamfer = primitive.edge_chamfer_smooth;

            let d_2d = eval_2d_profile(edge_profile, Vector2::new(p.x, p.y), size_xy, corner_r);
            let d_z = p.z.abs() - depth;
            let d_ext = d_2d.max(d_z).min(0.0) + Vector2::new(d_2d.max(0.0), d_z.max(0.0)).norm();
            if chamfer > 0.0 { d_ext - chamfer } else { d_ext }
        }
        "lathe" => {
            let size_xy = Vector2::new(primitive.extra_params[0], primitive.extra_params[1]);
            let offset = primitive.extra_params[2];
            let corner_r = primitive.extra_params[3];
            let q = Vector2::new(Vector2::new(p.x, p.z).norm() - offset, p.y);
            eval_2d_profile(edge_profile, q, size_xy, corner_r)
        }
        ...
        "bezier_curve" => {
            sd_bezier_taper(
                ...
                Vector3::new(primitive.extra_params[0], primitive.extra_params[1], primitive.extra_params[2]),
                Vector3::new(primitive.gyroid_params[0], primitive.gyroid_params[1], primitive.gyroid_params[2]),
                primitive.extra_params[3],
                primitive.gyroid_params[3],
            )
        }
        "mesh" => {
...
fn apply_primitive_to_accum(acc: &mut EvalAccum, d_prim: f32, prim: &SdfPrimitive) {
    if !acc.initialized {
        acc.d = if prim.operation == 0 { d_prim } else { 1e10 };
        acc.color = prim.color;
        acc.metallic = prim.metallic;
        acc.roughness = prim.roughness;
    ...
    match prim.operation {
        0 => {
            let h = (0.5 + 0.5 * (acc.d - d_prim) / k).clamp(0.0, 1.0);
            acc.d = apply_profile_union_cpu(acc.d, d_prim, profile, k, cs);
            acc.color = [
                lerp(acc.color[0], prim.color[0], h),
                lerp(acc.color[1], prim.color[1], h),
                lerp(acc.color[2], prim.color[2], h),
            ];
            acc.metallic = lerp(acc.metallic, prim.metallic, h);
            acc.roughness = lerp(acc.roughness, prim.roughness, h);
        }
        1 => {
            let h = (0.5 + 0.5 * (acc.d + d_prim) / k).clamp(0.0, 1.0);
            acc.d = apply_profile_sub_cpu(acc.d, d_prim, profile, k, cs);
            acc.color = [
                lerp(prim.color[0], acc.color[0], h),
                lerp(prim.color[1], acc.color[1], h),
                lerp(prim.color[2], acc.color[2], h),
            ];
            acc.metallic = lerp(prim.metallic, acc.metallic, h);
            acc.roughness = lerp(prim.roughness, acc.roughness, h);
        }
        2 => {
            let h = (0.5 + 0.5 * (d_prim - acc.d) / k).clamp(0.0, 1.0);
            acc.d = apply_profile_int_cpu(acc.d, d_prim, profile, k, cs);
            acc.color = [
                lerp(acc.color[0], prim.color[0], h),
                lerp(acc.color[1], prim.color[1], h),
                lerp(acc.color[2], prim.color[2], h),
...
fn union_layer_into_scene(scene: &mut EvalAccum, layer: EvalAccum, k_in: f32, profile: u32, cs: f32) {
    ...
    let h = (0.5 + 0.5 * (scene.d - layer.d) / k).clamp(0.0, 1.0);
    scene.d = apply_profile_union_cpu(scene.d, layer.d, profile, k, cs);
    scene.color = [
        lerp(scene.color[0], layer.color[0], h),
        lerp(scene.color[1], layer.color[1], h),
        lerp(scene.color[2], layer.color[2], h),
    ];
    scene.metallic = lerp(scene.metallic, layer.metallic, h);
    scene.roughness = lerp(scene.roughness, layer.roughness, h);
}

// V16.2.1: プリミティブ1個ぶんの評価（レイアウト -> デフォーム -> 形状 -> Shell -> ノイズ）。
// GPU 側 (common.wgsl) と同じ処理を、ミラーの「どちら側を見るか」だけ差し替えられる形にした。
// msign が 0 の成分は従来どおり abs() で折り返す。
fn eval_primitive_from_local(
    prim: &SdfPrimitive,
    local_p_orig: Vector3<f32>,
    bvh: &Option<MeshBvh>,
    msign: Vector3<f32>,
) -> f32 {
    ...
    if (mode_flags & 1) != 0 {
        ...
        if (mask & 1) != 0 { local_p.x = (if msign.x != 0.0 { msign.x * local_p.x } else { local_p.x.abs() }) - offset; }
        if (mask & 2) != 0 { local_p.y = (if msign.y != 0.0 { msign.y * local_p.y } else { local_p.y.abs() }) - offset; }
        if (mask & 4) != 0 { local_p.z = (if msign.z != 0.0 { msign.z * local_p.z } else { local_p.z.abs() }) - offset; }
    }
    ...
    let mut d_prim = calculate_primitive_sdf(local_p, prim, bvh);
...
// Mirror Blend: 0 なら従来どおり折り返し1回（形は V16.2.0 と同じ）。0 より大きい
// ときは有効な軸ぶん側を分けて評価し、プリミティブのブレンド形状で結合する。
fn eval_primitive_mirrored(prim: &SdfPrimitive, local_p_orig: Vector3<f32>, bvh: &Option<MeshBvh>) -> f32 {
    let mode_flags = prim.layout_data1[0] as u32;
    let mask = (mode_flags >> 8) & 0xF;
    let blend = prim.mirror_blend;
    if (mode_flags & 1) == 0 || mask == 0 || blend <= 0.0001 {
        return eval_primitive_from_local(prim, local_p_orig, bvh, Vector3::new(0.0, 0.0, 0.0));
    }
    ...
    for i in 0..8u32 {
        ...
        let ny = (i & 2) != 0;
        let nz = (i & 4) != 0;
        if nx && (mask & 1) == 0 { continue; }
        if ny && (mask & 2) == 0 { continue; }
        if nz && (mask & 4) == 0 { continue; }
        let sgn = Vector3::new(
            if nx { -1.0 } else { 1.0 },
            if ny { -1.0 } else { 1.0 },
            if nz { -1.0 } else { 1.0 },
        );
        let ds = eval_primitive_from_local(prim, local_p_orig, bvh, sgn);
        d = Some(match d {
            None => ds,
            Some(prev) => apply_profile_union_cpu(prev, ds, profile, k, cs),
        });
    }
    d.unwrap_or(1e9)
}
...
fn get_scene_sdf_with_color(p: Vector3<f32>, primitives: &[SdfPrimitive], bvhs: &[Option<MeshBvh>], symmetry: u32) -> (f32, [f32; 3], f32, f32) {
    let mut scene = EvalAccum::empty();
    let mut layer = EvalAccum::empty();
    let mut current_layer_id = 0u32;
    let mut layer_k = 0.0001f32;
    let mut layer_profile = 0u32;
    let mut layer_cs = 0.0f32;
    ...
    for (i, prim) in primitives.iter().enumerate() {
        ...
        let d_prim = eval_primitive_mirrored(prim, local_p_orig, &bvhs[i]);

        if prim.layer_id > 0 {
            if current_layer_id != prim.layer_id {
                if current_layer_id != 0 {
                    union_layer_into_scene(&mut scene, layer, layer_k, layer_profile, layer_cs);
            ...
            apply_primitive_to_accum(&mut scene, d_prim, prim);
        }
    }
    if current_layer_id != 0 {
        union_layer_into_scene(&mut scene, layer, layer_k, layer_profile, layer_cs);
    }
    (scene.d, scene.color, scene.metallic, scene.roughness)
}
...
#[pyfunction]
#[pyo3(signature = (primitives, res, size, symmetry=0, weld_threshold=0.0, chunk_cells=32))]
fn request_chunked_mesh_update(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<bool> {
    if IS_UPDATING.load(Ordering::SeqCst) {
        return Ok(false);
    }
    ...
    std::thread::spawn(move || {
        ...
        let result = generate_sdf_mesh_chunked_cpu_mc(primitives, res, size, symmetry, weld_threshold, chunk_cells);
        if let Ok(data) = result {
            let mut res_lock = match MESH_RESULT.lock() {
                Ok(lock) => lock,
                Err(poisoned) => poisoned.into_inner(),
...
#[pyfunction]
#[pyo3(signature = (primitives, res, size, symmetry=0, weld_threshold=0.0, chunk_cells=64))]
fn request_gpu_chunked_mesh_update(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<bool> {
    if IS_UPDATING.load(Ordering::SeqCst) {
        return Ok(false);
    }
    ...
    std::thread::spawn(move || {
        let _guard = UpdatingGuard;
        let result = generate_sdf_mesh_gpu_chunked_mc(primitives, res, size, symmetry, weld_threshold, chunk_cells);
        if let Ok(data) = result {
            let mut res_lock = match MESH_RESULT.lock() {
                Ok(lock) => lock,
                Err(poisoned) => poisoned.into_inner(),
...
#[pyfunction]
#[pyo3(signature = (primitives, res, size, symmetry=0, weld_threshold=0.0, chunk_cells=64))]
fn request_gpu_chunked_dc_update(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<bool> {
    if IS_UPDATING.load(Ordering::SeqCst) {
        return Ok(false);
    }
    ...
    std::thread::spawn(move || {
        let _guard = UpdatingGuard;
        let result = generate_sdf_mesh_gpu_chunked_dc(primitives, res, size, symmetry, weld_threshold, chunk_cells);
        if let Ok(data) = result {
            let mut res_lock = match MESH_RESULT.lock() {
                Ok(lock) => lock,
                Err(poisoned) => poisoned.into_inner(),
...
fn generate_sdf_mesh_gpu_chunked_mc(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<(Vec<f32>, Vec<u32>)> {
    let gpu_lock = GPU_CONTEXT.lock().unwrap();
    let gpu_ctx = match gpu_lock.as_ref() {
        Some(ctx) => ctx,
        None => return generate_sdf_mesh_chunked_cpu_mc(primitives, res, size, symmetry, weld_threshold, chunk_cells),
    };
    if primitives.iter().any(|p| p.shape_type == "mesh") {
        return generate_sdf_mesh_chunked_cpu_mc(primitives, res, size, symmetry, weld_threshold, chunk_cells);
    }

    let chunk_cells = if res < 32 {
        res.max(8)
    } else {
        chunk_cells.max(32).min(res)
    };
    let chunks_per_axis = (res + chunk_cells - 1) / chunk_cells;
    let total_chunks = chunks_per_axis.saturating_mul(chunks_per_axis).saturating_mul(chunks_per_axis);
    if total_chunks > 2048 {
        if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
            *diag = format!("health=BLOCK_OR_PRIM_FALLBACK; algo=GPU_CHUNKED_MC; reason=too_many_chunks; res={}; chunk_cells={}; chunks={}", res, chunk_cells, total_chunks);
        }
        return generate_sdf_mesh_chunked_cpu_mc(primitives, res, size, symmetry, weld_threshold, chunk_cells);
    }

    let gpu_prims: Vec<gpu::GpuPrimitive> = primitives.iter().map(|p| convert_to_gpu_prim(p)).collect();
    let prim_aabbs = build_gpu_primitive_aabbs(&primitives, symmetry);
    ...
    let config = GpuConfig {
        ...
        max_tris: 0,
        _pad: 0,
        domain_origin: [0.0, 0.0, 0.0, 0.0],
    };

    let (v, i, stats) = futures::executor::block_on(gpu_ctx.generate_mesh_gpu_chunked_mc(&gpu_prims, &bvh_nodes, config, chunk_cells as u32));
    let (v_w, i_w) = if weld_threshold > 0.0 {
        weld_mesh(v, i, (size / res as f32) * weld_threshold)
    } else {
        (v, i)
    };
    gpu_ctx.swap_buffers();
    if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
        *diag = format!("{}; welded_verts={}; welded_faces={}; chunk_cells={}; ghost_cells=1; weld_scale={:.4}", stats.summary(), v_w.len() / 11, i_w.len() / 3, chunk_cells, weld_threshold);
    }
    Ok((v_w, i_w))
}

fn generate_sdf_mesh_gpu_chunked_dc(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<(Vec<f32>, Vec<u32>)> {
    let gpu_lock = GPU_CONTEXT.lock().unwrap();
    let gpu_ctx = match gpu_lock.as_ref() {
        Some(ctx) => ctx,
        None => {
            if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
                *diag = String::from("health=EMPTY_RESULT; algo=GPU_CHUNKED_DC; reason=no_gpu_context");
            }
            return Ok((vec![], vec![]));
        }
    };
    if primitives.iter().any(|p| p.shape_type == "mesh") {
        if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
            *diag = String::from("health=BLOCK_OR_PRIM_FALLBACK; algo=GPU_CHUNKED_DC; reason=mesh_primitive_unsupported");
        }
        return Ok((vec![], vec![]));
    }

    let chunk_cells = if res < 64 {
        res.max(8)
    } else {
        chunk_cells.max(64).min(res)
    };
    let chunks_per_axis = (res + chunk_cells - 1) / chunk_cells;
    let total_chunks = chunks_per_axis.saturating_mul(chunks_per_axis).saturating_mul(chunks_per_axis);
    if total_chunks > 512 {
        if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
            *diag = format!("health=EMPTY_RESULT; algo=GPU_CHUNKED_DC; reason=too_many_chunks; res={}; chunk_cells={}; chunks={}", res, chunk_cells, total_chunks);
        }
        return Ok((vec![], vec![]));
    }
    ...
    let (v, i, stats) = futures::executor::block_on(gpu_ctx.generate_mesh_gpu_chunked_dc(&gpu_prims, &bvh_nodes, config, chunk_cells as u32));
    let (v_w, i_w) = if weld_threshold > 0.0 {
        weld_mesh(v, i, (size / res as f32) * weld_threshold)
    } else {
        (v, i)
    ...
    gpu_ctx.swap_buffers();
    if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
        *diag = format!("{}; welded_verts={}; welded_faces={}; chunk_cells={}; ghost_cells=1; weld_scale={:.4}; experimental=true", stats.summary(), v_w.len() / 11, i_w.len() / 3, chunk_cells, weld_threshold);
    }
    Ok((v_w, i_w))
}

/// Twist デフォーム用の AABB 拡張。捻り軸まわりの回転で形状が軸に垂直な平面内を
/// 掃くため、その平面を構成する2軸を外接円半径まで広げる（軸方向は不変）。
/// 評価側 (`get_scene_sdf_with_color` の d_type==3) と対になる処理。
fn expand_aabb_for_twist(
    aabb_min: &mut Vector3<f32>,
    aabb_max: &mut Vector3<f32>,
    d_axis: u32,
    angle: f32,
) {
    if angle.abs() <= 0.0001 {
    ...
    // 軸に垂直な2成分の最大絶対値から外接円半径を求める
    let (i, j) = match d_axis {
        0 => (1usize, 2usize), // X軸まわり -> YZ平面
        1 => (2usize, 0usize), // Y軸まわり -> ZX平面
        _ => (0usize, 1usize), // Z軸まわり -> XY平面
    };
    let ext_i = aabb_min[i].abs().max(aabb_max[i].abs());
    let ext_j = aabb_min[j].abs().max(aabb_max[j].abs());
    let r = ext_i.hypot(ext_j);
    aabb_min[i] = aabb_min[i].min(-r);
    aabb_max[i] = aabb_max[i].max(r);
    aabb_min[j] = aabb_min[j].min(-r);
    aabb_max[j] = aabb_max[j].max(r);
}

fn build_gpu_primitive_aabbs(primitives: &[SdfPrimitive], symmetry: u32) -> Vec<(Vector3<f32>, Vector3<f32>)> {
    ...
    for prim in primitives {
        ...
        match prim.shape_type.as_str() {
            "sphere" => local_factor = Vector3::new(prim.radius, prim.radius, prim.radius),
            "torus" => {
            ...
            "ngon_prism" => local_factor = Vector3::new(prim.extra_params[0], prim.extra_params[0], prim.extra_params[2]),
            "ellipsoid" => local_factor = Vector3::new(prim.extra_params[0], prim.extra_params[1], prim.extra_params[2]),
            "rounded_cylinder" => {
            ...
            "cut_sphere" => local_factor = Vector3::new(prim.radius, prim.radius, prim.radius),
            "math_field" => {
                let e = prim.extra_params[3].max(0.01f32);
                local_factor = Vector3::new(e, e, e);
            }
            "extrude" => {
                let sx = prim.extra_params[0];
                let sy = prim.extra_params[1];
                let sz = prim.extra_params[2];
                local_factor = Vector3::new(sx, sy, sz);
            }
            "lathe" => {
                let sx = prim.extra_params[0];
                let sy = prim.extra_params[1];
                let offset = prim.extra_params[2];
                let r_total = offset + sx;
                local_factor = Vector3::new(r_total, sy, r_total);
            }
            "bezier_curve" => {
                let bx = prim.extra_params[0].abs();
                let by = prim.extra_params[1].abs();
                let bz = prim.extra_params[2].abs();
                let cx = prim.gyroid_params[0].abs();
                let cy = prim.gyroid_params[1].abs();
                let cz = prim.gyroid_params[2].abs();
                let r_max = prim.extra_params[3].max(prim.gyroid_params[3]);
                let max_x = bx.max(cx) + r_max;
                let max_y = by.max(cy) + r_max;
                let max_z = bz.max(cz) + r_max;
                local_factor = Vector3::new(max_x, max_y, max_z);
            }
            _ => {}
        }
        ...
        let half_ext = Vector3::new(
            (prim.size[0] * local_factor.x).abs(),
            (prim.size[1] * local_factor.y).abs(),
            (prim.size[2] * local_factor.z).abs(),
        );
        let mut aabb_min = -half_ext;
        let mut aabb_max = half_ext;

        let edge_pad = if prim.shape_type == "math_field" { 0.0 } else { prim.edge_profile_size };
        let extra_pad = prim.smoothness + prim.noise_strength + prim.shell_thickness + edge_pad;
        ...
        for si in 0..4u32 {
            ...
            if d_type == 1 {
                ...
                // Twist: 捻り軸まわりに回転するため、軸に垂直な2軸は外接円半径まで広げる
                expand_aabb_for_twist(&mut aabb_min, &mut aabb_max, d_axis, sd[0]);
            } else if d_type == 4 {
                let factor = sd[0];
                if factor > 0.0 {
        ...
        // Radial/Spiral の各インスタンスは Individual/Step Rotation で個別に追加回転される
        // （calculate_primitive_sdf 側は無条件に適用）。AABB 側は prim.rotation しか見ないため、
        // 回転しても不変な「ローカルボックスの外接球半径」を先に控えておき、Radial 展開時の
        // footprint 計算に織り込むことで、非対称形状（bezier_curve等）が実際の向きより
        // 小さく見積もられてチャンクが誤カリングされる（メッシュ欠損）のを防ぐ
        let rot_invariant_radius = (aabb_max - aabb_min).norm() * 0.5;
        ...
        if (flags & 1) != 0 {
            ...
            let mut m_min = aabb_min;
            let mut m_max = aabb_max;
            if (mask & 1) != 0 {
        ...
        if (flags & 2) != 0 || (flags & 4) != 0 {
            ...
            // Radial Axis (evaluate側の `let axis = (mode_flags >> 20) & 3;` と同一の読み方)。
            // 従来は常に X/Y をリング面・Z を奥行き軸として扱っており、Radial Axis が X/Y の
            // 場合にリング半径が正しい軸に加算されていなかった（穴の原因の一つ）
            let axis = (packed1 >> 20) & 3;
            let radius = prim.layout_data1[2].abs();
            let pitch = prim.layout_data1[3].abs();
            let ring_extent = rot_invariant_radius + radius;
            let spiral_total = pitch * count.max(1.0);
            let depth_extent = rot_invariant_radius + spiral_total;
            let rad_extent = match axis {
                0 => Vector3::new(depth_extent, ring_extent, ring_extent),
                1 => Vector3::new(ring_extent, depth_extent, ring_extent),
                _ => Vector3::new(ring_extent, ring_extent, depth_extent),
            };
        ...
        if (symmetry & 1) != 0 {
            ...
            aabb_min.x = -x_max; aabb_max.x = x_max;
        }
        if (symmetry & 2) != 0 {
            let y_max = aabb_min.y.abs().max(aabb_max.y.abs());
            aabb_min.y = -y_max; aabb_max.y = y_max;
        }
        if (symmetry & 4) != 0 {
            let z_max = aabb_min.z.abs().max(aabb_max.z.abs());
            aabb_min.z = -z_max; aabb_max.z = z_max;
        }
...
fn generate_sdf_mesh_chunked_cpu_mc(primitives: Vec<SdfPrimitive>, res: usize, size: f32, symmetry: u32, weld_threshold: f32, chunk_cells: usize) -> PyResult<(Vec<f32>, Vec<u32>)> {
    ...
    let chunk = chunk_cells.max(8).min(res.max(8));
    ...
    let mut x0 = 0;
    while x0 < res {
        let x1 = (x0 + chunk).min(res);
        chunk_ranges.push((x0, x1));
        x0 = x1;
    }

    let chunk_data: Vec<(Vec<f32>, Vec<u32>)> = chunk_ranges.into_par_iter().map(|(x_start, x_end)| {
            ...
            for y in 0..res {
                for z in 0..res {
                    ...
                    for i in 0..8 {
                        ...
                        let p = Vector3::new(
                            (ix as f32 - half_res) * step,
                            (iy as f32 - half_res) * step,
                            (iz as f32 - half_res) * step
                        );
                        values[i] = get_scene_sdf_with_color(p, &primitives, &mesh_bvhs, symmetry).0;
    ...
    if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
        *diag = format!(
            "health=OK; algo=CPU_CHUNKED_MC; active_blocks=chunked; faces={}; verts={}; indices={}; chunk_cells={}; elapsed_ms={}",
            final_indices.len() / 3,
            final_verts.len() / 11,
            final_indices.len(),
            chunk,
...
fn generate_sdf_mesh_internal(primitives: Vec<SdfPrimitive>, res: usize, size: f32, use_dc: bool, symmetry: u32, weld_threshold: f32) -> PyResult<(Vec<f32>, Vec<u32>)> {
    ...
    if can_use_gpu {
        ...
        for prim in &primitives {
            ...
            match prim.shape_type.as_str() {
                ...
                "bezier_curve" => {
                    ...
                    let cz = prim.gyroid_params[2].abs();
                    let r_max = prim.extra_params[3].max(prim.gyroid_params[3]);
                    local_factor = Vector3::new(bx.max(cx) + r_max, by.max(cy) + r_max, bz.max(cz) + r_max);
                }
                _ => {}
            }
            ...
            for si in 0..4u32 {
                ...
                if d_type == 1 { // Elongate
                    ...
                    // 捻り軸に垂直な2軸を外接円半径まで広げる（評価側 d_type==3 と対）
                    expand_aabb_for_twist(&mut aabb_min, &mut aabb_max, d_axis, sd[0]);
                } else if d_type == 4 { // Taper
                    let factor = sd[0];
                    if factor > 0.0 {
            ...
            // Radial/Spiral の各インスタンスは Individual/Step Rotation で個別に追加回転される
            // （calculate_primitive_sdf 側は無条件に適用）。回転しても不変な「ローカルボックスの
            // 外接球半径」を先に控えておき、Radial 展開時の footprint 計算に織り込むことで、
            // 非対称形状（bezier_curve等）のチャンク誤カリング（メッシュ欠損）を防ぐ
            let rot_invariant_radius = (aabb_max - aabb_min).norm() * 0.5;
        ...
        gpu_ctx.swap_buffers();
        if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
            *diag = format!("{}; welded_verts={}; welded_faces={}", stats.summary(), v_w.len() / 11, i_w.len() / 3);
        }
        return Ok((v_w, i_w));
    } else {
    ...
    if let Ok(mut diag) = LAST_MESH_DIAGNOSTICS.lock() {
        let algo = if use_dc { "CPU_DC" } else { "CPU_MC" };
        *diag = format!(
            "health=OK; algo={}; active_blocks=cpu; faces={}; verts={}; indices={}; elapsed_ms={}",
            algo,
            final_indices.len() / 3,
            final_verts.len() / 11,
            final_indices.len(),
...
/// V15.9.9.1: 直近のDCパイプラインコンパイル失敗の診断文字列を取得する (成功時/未実行時は None)
#[pyfunction]
fn get_dc_last_error() -> Option<String> {
    let ctx_lock = GPU_CONTEXT.lock().unwrap();
    ctx_lock.as_ref().and_then(|ctx| ctx.get_dc_last_error())
}

/// V15.9.9.1: 診断ログファイルの絶対パスを取得する (未初期化時は None)
#[pyfunction]
fn get_mesh_diagnostics() -> String {
    match LAST_MESH_DIAGNOSTICS.lock() {
        Ok(diag) => diag.clone(),
...
fn rust_gpu_sdf(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(request_gpu_chunked_mesh_update, m)?)?;
    m.add_function(wrap_pyfunction!(request_gpu_chunked_dc_update, m)?)?;
    m.add_function(wrap_pyfunction!(request_chunked_mesh_update, m)?)?;
    ...
    m.add_function(wrap_pyfunction!(get_dc_last_error, m)?)?;
    m.add_function(wrap_pyfunction!(get_mesh_diagnostics, m)?)?;
    m.add_function(wrap_pyfunction!(get_log_path, m)?)?;
    Ok(())
}
fn convert_to_gpu_prim(p: &SdfPrimitive) -> gpu::GpuPrimitive {
    ...
    gpu::GpuPrimitive {
        ...
        color_b_and_extra: [p.color[2], p.blend_profile as f32, p.chamfer_smooth, p.layer_id as f32],
        ...
        layer_params: [p.layer_smoothness, p.layer_blend_profile as f32, p.layer_chamfer_smooth, p.mirror_blend],
