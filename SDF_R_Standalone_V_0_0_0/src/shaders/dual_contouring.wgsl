fn get_corner(i: u32) -> vec3<f32> {
    switch(i) {
        case 0u: { return vec3<f32>(0,0,0); }
        case 1u: { return vec3<f32>(1,0,0); }
        case 2u: { return vec3<f32>(1,1,0); }
        case 3u: { return vec3<f32>(0,1,0); }
        case 4u: { return vec3<f32>(0,0,1); }
        case 5u: { return vec3<f32>(1,0,1); }
        case 6u: { return vec3<f32>(1,1,1); }
        case 7u: { return vec3<f32>(0,1,1); }
        default: { return vec3<f32>(0,0,0); }
    }
}

fn get_edge(i: u32) -> vec2<u32> {
    switch(i) {
        case 0u: { return vec2<u32>(0,1); }
        case 1u: { return vec2<u32>(1,2); }
        case 2u: { return vec2<u32>(2,3); }
        case 3u: { return vec2<u32>(3,0); }
        case 4u: { return vec2<u32>(4,5); }
        case 5u: { return vec2<u32>(5,6); }
        case 6u: { return vec2<u32>(6,7); }
        case 7u: { return vec2<u32>(7,4); }
        case 8u: { return vec2<u32>(0,4); }
        case 9u: { return vec2<u32>(1,5); }
        case 10u: { return vec2<u32>(2,6); }
        case 11u: { return vec2<u32>(3,7); }
        default: { return vec2<u32>(0,0); }
    }
}

// get_scene_normal is now provided by common.wgsl
...
fn vertex_pass(@builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    let clear_base = u32(b_ptr) * 512u; let tid = lid.x + lid.y * 8u + lid.z * 64u;
    ...
    for (var z_off = 0u; z_off < 8u; z_off += 4u) {
        ...
        for (var i = 0u; i < 8u; i++) {
            vals[i] = get_scene_dist_indexed(p_min + get_corner(i) * step, b_ptr_u);
        ...
        for (var i = 0u; i < 12u; i++) {
            let edge = get_edge(i);
            let v1 = edge.x; let v2 = edge.y;
            if ((vals[v1] <= 0.0) != (vals[v2] <= 0.0)) {
                let t = clamp(vals[v1] / (vals[v1] - vals[v2] + 1e-10), 0.0, 1.0);
                let p = p_min + mix(get_corner(v1), get_corner(v2), t) * step;
        ...
        for (var axis = 0u; axis < 3u; axis++) {
            ...
            if ((get_scene_dist_indexed(p1, b_ptr_u) <= 0.0) != (get_scene_dist_indexed(p1 + p2_off * step, b_ptr_u) <= 0.0)) {
                ...
                for (var i = 0u; i < 4u; i++) {
                    ...
                    let v_val = block_data[u32(b_ptr_q) * 512u + (u32(q.x % 8) | (u32(q.y % 8) << 3u) | (u32(q.z % 8) << 6u))];
                ...
                if (valid) {
                    ...
                    if (get_scene_dist_indexed(p1, b_ptr_u) <= 0.0) {
                        ...
                        indices[base+3]=v_indices[1]; indices[base+4]=v_indices[3]; indices[base+2]=v_indices[2];
                    } else {
