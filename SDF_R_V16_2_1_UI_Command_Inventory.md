# SDF.R V16.2.1 UI / Command Inventory

対象: **Rust-GPU-SDF / SDF.R V16.2.1** \| 作成日: 2026-07-19（V16.2.1 追記: 2026-09-09） \| 用途: UIボタン・主要コントロールの棚卸し資料
...
| Mirror Blend | 数値 `mirror_blend` （V16.2.1） | ミラーの継ぎ目を丸める。**既定 0.0**。 | Mesh再計算 | **0のときは従来と完全に同じ経路**。>0で両側を別々に評価するため、評価コストが1軸あたり約1.7倍、軸を増やすごとに倍。プリミティブの `Smoothness` はこの継ぎ目に効かないため、丸めたいときはこちらを使う。**Radial / Grid は未対応**。プレビューは近似表示。 |
...
Source basis: `rust_gpu_sdf_addon/ui.py`, `operators.py`, `properties.py`, `constants.py` in Rust-GPU-SDF-V16.2.1.
