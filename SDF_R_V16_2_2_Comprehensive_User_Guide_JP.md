# SDF.R V16.2.2 Comprehensive User Guide

対象: Rust-GPU-SDF / SDF.R V16.2.2  
用途: 公式チュートリアル、操作マニュアル、機能棚卸し、動画台本の下敷き  
作成日: 2026-07-22（V16.0.7版）  
更新日: 2026-09-12（V16.2.2対応。第15章 Mirror Blend のゴーストプレビューの記述を改訂）

> **V16.1.3以前から更新した方へ**: Layer Boundaryの仕様が変わりました。グループの範囲がDividerの
> 「下」から「上」になり、合流の強さがDivider自身の `Layer Blend`（既定0.0＝硬い境界）から
> 決まるようになっています。既存ファイルの見た目が変わる場合があります。詳細は第10章を参照してください。
...
このドキュメントは、単なるボタン一覧ではなく、読めばSDF.Rの作業の流れが想像でき、実際に触り始められることを目標にしています。各UIの意味、再計算の起き方、どの場面でどの機能を使うべきか、V16.1.0で追加されたCurve Sync、V16.2.0で仕様を整理したLayer Boundary、V16.2.1で追加されたSDFツリーやMath Fieldの使いどころまで、できるだけ網羅的に整理します。
...
|----|----|----|----|
| Windows | `SDF_R_16_1_1.zip` | x86-64 | 標準 |
| macOS | `SDF_R_16_1_1_MAC.zip` | Apple Silicon (arm64) | experimental test build |
| Linux | `SDF_R_16_1_1_LINUX.zip` | x86-64 | experimental test build |
...
| 更新元 | cache削除 |
|----|----|
| V16.2.1 → V16.2.2 | **手動削除は不要**。GPUシェーダーのコードは変わっていないため、**再構築は走りません**。初回起動もこれまでどおりです |
| V16.2.0 → V16.2.1 | **手動削除は不要**。ただしGPUシェーダーのコードが変更されているため、**初回起動で一度だけ再構築が走ります**（下記参照） |
| V16.1.x以前 → V16.2.1 | **手動削除は不要**（起動失敗時は自動で削除・再試行されます） |

### 初回起動とWarm-up

初回利用時、cache削除後、および**GPUシェーダーのコードが変わったバージョンへ更新した直後**は、shaderとcompute pipelineの準備に時間がかかります。Blenderが一時的に反応しづらく見える場合がありますが、待てば完了します。

所要時間はGPUによって大きく変わります。

| 環境 | キャッシュ無効時（初回・更新直後） | 2回目以降 |
|----|----|----|
| ディスクリートGPU | 15〜45秒 | 数秒 |
| 内蔵GPU（例: Intel Iris Xe） | **数分**（実測194秒） | 1秒前後 |

**内蔵GPUで数分かかるのは異常ではありません。** 判断に迷ったらコンソールを見てください。
処理中は `Compiling MC Pipeline...` が表示され、完了すると `GPU Engine Ready!` が出ます。
この間にBlenderを終了しないでください。
...
| セクション | 主な役割 |
|----|----|
| Tree | 編集対象のSDFツリーの選択と追加（V16.2.1で追加。詳しくは第4-B章） |
...
| Tree Reference | 別のSDFツリーを取り込む（V16.2.1で追加。詳しくは第4-B章） |
...
- SDFツリーとは何か、なぜ分けるのか
- ツリーの追加と切り替え
- Tree Reference で、はめ合いを作る
- 分けたときのプレビューの挙動

V16.2.1 から、**1つのシーンに SDF ツリーを何本でも置けるようになりました**。

### SDFツリーとは

**1つのパーツ用コレクションと、そこから作られる1つの結果メッシュ**の組を、SDFツリーと呼びます。
ツリーごとに、独立した設定（解像度、ドメイン、対称など）と、独立した出力オブジェクトを持ちます。

V16.2.0 までは、シーンが持てるツリーは1本だけでした。つまり**出力メッシュも必ず1つ**で、
モデルのパーツを別オブジェクトにする手段がありませんでした。

### なぜ分けるのか

**同じツリーの中では、重なった形は必ず1つの繋がった立体になります。**

第10章の Layer Boundary は、グループが周囲に**溶ける**のを止める機能です。しかし合流そのものは
必ず Union なので、重なっていれば繋がった1つのメッシュのままです。ここは V16.2.0 でも変わりません。

**別のツリー同士は、そもそも一切干渉しません。** 結果メッシュが最初から別オブジェクトなので、
分割作業も要りませんし、別々のマテリアルも普通に割り当てられます。

| | Layer Boundary（第10章） | ツリー分割（この章） |
|---|---|---|
| 結果メッシュ | 1つ | **パーツごとに1つ** |
...
パネル最上段に **`Tree`** 行があります。**ツリーが1本のときも常に表示されます。**

| 表示 | 役割 |
|---|---|
| ドロップダウン | 編集対象のツリーを選ぶ。以下のパネル全体がこれに追従する |
| **＋** | 新しいツリーを追加する。既存のツリーは退避されない |
| `Parts:` | 現在のツリーのパーツ用コレクション名 |
| `N trees` | シーン内のツリー本数（2本以上のときだけ表示） |

**ビューポートでパーツをクリックするだけでも切り替わります。** そのパーツが属するツリーが
自動的に編集対象になります。

1本目のツリーは、従来どおり `SDF_Collection` / `SDF_Result` という名前のままです。
2本目以降が連番になります。既存ファイルを開いても名前は変わりません。

> **`New SDF Workspace` と `＋` の違い**
> `New SDF Workspace` は、これまでの作業を履歴へ**退避してから**新しく始めるボタンです。
> `＋` は退避せず、**今のツリーを残したまま**もう1本増やします。

### Finalize はアクティブなツリーだけ

**`Finalize (Bake)` は、選択中のツリー1本にだけ効きます。** 他のツリーが残っている間は
Live Update も切れず、そのまま編集を続けられます。最後の1本を確定したときだけ、
従来どおり Live Update が切れます。

### Tree Reference（はめ合いを作る）

**あるツリーから、別のツリーの形を読み取れます。** 蓋と瓶のように、片方の形を相手に
反映させたいときに使います。

1. パーツを選んだ状態で **`Add Tree Reference`** を押します。スタックにEmptyが1つ追加されます。
2. Emptyの `Tree` に、取り込みたい相手のツリーを指定します。
3. `Mode` を選びます。

| Mode | 結果 |
|---|---|
| **Blend** | 参照先のツリーが、ひとかたまりとして結合される |
| **Subtract** | 参照先のツリーの形が、こちらから削られる |

はめ合いに使うのは `Subtract` です。瓶のツリーから蓋のツリーを Subtract すれば、
蓋の形のくぼみが空きます。**蓋を編集すると、くぼみも自動で追従します。**

#### 精度について

- **`Blend` は厳密です。**
- **`Subtract` は、参照先が Union だけで組まれていれば厳密です。** 参照先が内部で
  `Subtract` や `Intersect` を使っている場合、その箇所は近似になります。
  この状態のときは、パネルに注意表示が出ます。

#### 使ううえでの注意

- **Emptyは動かさないでください。** 作成時は参照先の真上に置かれるので、そのままなら位置が一致します。
  動かすと、その分だけ参照先がずれます（意図的にずらしたいときには使えます）。
- **参照の入れ子は1段までです。** 参照先がさらに別のツリーを参照していても、そこは辿りません。
- 自己参照や相互参照は無視されます（無限ループを避けるため）。

### プレビューはアクティブなツリーだけ

**ゴーストプレビューは、選択中のツリー1本だけを描きます。**

ドメインと対称の設定はツリーごとに持つもので、設定の違うツリーを1枚の絵に混ぜると
誤った表示になるためです。またプレビューは、視点を動かすたびに毎フレーム走る唯一の処理です。

ただし、**見えなくなるわけではありません。各ツリーの結果メッシュは実体のあるオブジェクトなので、
常に全部表示されています。** ライブプレビューだけが1本に絞られている、という理解で問題ありません。
はめ合いの確認は、結果メッシュで行ってください。

### 速度について

**分けると速くなります。** 遅くなりそうに思えますが、逆です。

1リクエストあたりのプリミティブ数が減り、疎ブロック検出も効きやすくなるためです。
216個の球・解像度128のモデルでの実測値です。
...
ツリーが増えるだけの常時コストもごくわずかです（16本でdepsgraphイベントあたり0.74ms）。
**本数に上限は設けていません。**
...
この章はSDF.Rで一番つまずきやすい場所です。ボタンの説明から入らず、**評価のしくみ**から順に積み上げます。

### まず「バケツは1つ」から

SDF.Rはスタックを上から順に読み、**1つのバケツ（累積器）に形を混ぜていきます**。

```text
01 Cylinder            → バケツに入れる
02 Sphere / Union      → バケツの中身とUnionで混ぜる
03 Box / Subtract      → バケツの中身から引く
```

大事なのは、**各Primitiveは「その時点でのバケツの中身」に対して操作する**ということです。03のSubtractは01と02が混ざった塊から引きます。特定の相手だけを狙って引くことはできません。

「Intersectが本体まで切ってしまう」「装飾のSubtractが本体まで削る」といった悩みは、すべてここから来ています。

### Collection Dividerの基本

`Add Collection Divider` を押すと、Stack内にDividerが追加されます。DividerはPrimitiveではありませんが、Stack上でグループの区切りとして機能します。

**Dividerは「ここまでが1グループ」を表す線です。Dividerより上（前のDividerまで）が、そのDividerのグループになります。**

主な用途:

- Stackを読みやすく分ける。
- 複数Primitiveをグループとして扱う。
- Group Layout（Mirror / Radial / Spiral / Grid / Jitter）をグループへ適用する。
- Layer Boundaryでローカル評価範囲を作る。

Dividerを置いただけではバケツは1つのままです。評価の挙動が変わるのは、次のLayer BoundaryをONにしたときです。
...
| Layer Boundary | このDividerより上のグループを、独立したLayerとして評価する |
| Layer Blend | そのLayerをシーンへ合流させるときのブレンド半径。既定0.0（硬い境界） |
| Profile | 合流時のブレンド形状（Round / Sharp / Soft / Tight / Chamfer） |
| Break Parent | グループのつながりをここで区切る（Layerにはしない） |
| Group Layout | Mirror、Radial、Spiral、Grid、Jitterをグループへ適用 |

Layer Blend と Profile は、Layer BoundaryがONのときだけ表示されます。

### Layer Boundary =「そのグループ専用のバケツをもう1つ用意する」

Layer BoundaryをONにすると、**そのグループだけ別のバケツで組み立ててから、出来上がった塊をメインのバケツへ1回だけ注ぎ込みます**。

大事なのは「1回だけ」の部分です。グループの中で何個の形を使っていようと、外から見れば**注ぎ込まれるのは完成した1つの塊**です。したがって:

- グループの中の形は、グループの外の形と**直接ブレンドしません**。
- グループの中のSubtractやIntersectは、グループの外に**届きません**（別のバケツなので触れない）。

### Layer Blend =「注ぎ込むときの柔らかさ」

その1回の注ぎ込みも、SDFなので滑らかに混ぜられます。その半径が **Layer Blend** です。

| 値 | 結果 |
|----|----|
| 0.00（既定） | 硬い境目。グループの塊がくっきり乗る |
| 0.10〜0.30 | 境目が丸く馴染む |

Layer Blendは**そのDivider自身の設定**です。他のDividerのグループには影響しません。

> **V16.1.3以前との違い**: 以前はこの値がGroupの先頭Primitiveの `Smoothness` から取られていました。
> その値はグループ内部の見た目には効かないため、「数値を動かしても内部は変わらず、外との合流だけが
> 変わる」という分かりにくい二重用途になっていました。V16.2.0でDivider専用の設定に分離しています。
> Primitive側の `Smoothness` とその継承の挙動は変わっていません。

### Subtractをどちら側に置くかで結果が変わる

同じSubtractでも、Dividerの上に置くか下に置くかで効果範囲が変わります。ここが理解の分かれ目です。

**ケースA: Subtractを仕切りより下に置く**

```text
01 Cylinder / Union
== Divider ==            ← Layer Boundary ON
02 Sphere / Subtract
```
...
1. Cylinderをレイヤーに入れる
2. レイヤーをシーンへ合流する（ここでグループが確定）
3. Sphereがシーン全体を削る

→ **シリンダーが削れます。** 合流後のシーンは普通のシーンなので、後続のPrimitiveは自由に操作できます。

**ケースB: Subtractを仕切りより上に置く**
...
1. Cylinderをレイヤーに入れる
2. Sphereがレイヤーの中のCylinderだけを削る
3. 削れた塊をシーンへ合流する

→ **グループの中だけが削れます。** シーンに他の形があっても無傷です。装飾やMath Fieldの範囲制御に使うのはこちらです。

### 複数のDividerを使う（本来の使い方）

Layer Boundaryの効力は、**分けたい部品ごとにDividerを立てて初めて現れます**。カタツムリを例にします。

```text
01 頭 A
02 頭 B
== Divider A ==          ← Layer Boundary ON, Layer Blend 0.00
03 目 L
04 目 R
05 まぶた / Subtract
== Divider B ==          ← Layer Boundary ON, Layer Blend 0.00
06 殻
07 殻の模様
== Divider C ==          ← Layer Boundary ON, Layer Blend 0.10
```

上から順に何が起きるか:

1. **01〜02** バケツに溜まる。まだ何も確定していない。
2. **Divider A** 上に溜まっていた01・02が1グループと確定。頭Aと頭Bは互いのSmoothnessで普通にブレンドし、出来た塊が**レイヤー1**としてシーンへ注がれる。バケツは空になる。
3. **03〜05** 新しいバケツに溜まる。**まぶたのSubtractが届くのはこのバケツの中だけ**。頭はすでにシーン側へ行っているので、まぶたをどれだけ大きくしても頭は削れない。
4. **Divider B** 目とまぶたの塊が**レイヤー2**としてシーンへ注がれる。頭と目が接する境目の柔らかさを決めるのは、**Divider BのLayer Blend**。0.00なので目がくっきり頭に乗る。
5. **06〜07** さらに新しいバケツへ。
6. **Divider C** 殻の塊が**レイヤー3**として注がれる。Layer Blend 0.10なので、殻だけは少し馴染む。

**Dividerの数だけレイヤーができ、それぞれの合流の柔らかさは、そのDivider自身が持っています。** Divider Aの設定が目に影響することはありません。

### 単独のLayer Boundaryはほぼ効かない

「Layer BoundaryをONにしたのに何も変わらない」という場合、原因はたいてい次の2つです。どちらも「相手がいないと意味がない」という同じことの裏表です。

**① 最初のレイヤーのLayer Blendは効きません。** レイヤー1が注がれる時点でシーンは空で、混ぜる相手がいないためそのまま入ります。頭のグループのLayer Blendをいじっても何も変わらないのはこのためで、不具合ではありません。効くのは2番目以降のレイヤーです。

**② レイヤーに入れなかった形は、合流後の塊に普通にブレンドします。** レイヤーが守ってくれるのは「注ぎ込まれるまで」であって、注ぎ込んだ後のシーンは普通のシーンです。目だけをグループにして頭をグループにしなかった場合、頭は自分のSmoothnessでシーン（＝目が乗った塊）に混ざりにいくので、結局ぬるっと繋がります。目のグループが悪いのではなく、頭が野放しだからです。

したがって実用上の指針はこうなります。

> **混ざらせたくない部品は、全部それぞれグループにする。**
> レイヤーに入れなかった形は、全体に対する「共通の接着剤」または「共通の削り取り」として働く。

裏を返すと、**最後のDividerより下に置いた形は、シーン全体に直接効きます**。全体をまとめて削る、全体に共通のディテールを馴染ませる、といった用途にはそこが正しい置き場所です。

### Layer Boundaryが向く用途

- 目・まぶた・殻など、隣り合う部品を融合させずに配置する。
...
**Layer Boundaryは「くっつかなくなる」機能ではありません。** レイヤーの合流は必ずUnionなので、形が重なっていれば繋がった1つの立体になります。止まるのは「境目がぬるっと溶けること」だけです。目を独立したオブジェクトとして扱いたい場合は、SDF出力オブジェクト自体を2つに分けてください。

**グループ全体をSubtractとして本体から引くことはできません。** 注ぎ込みはUnion固定です。「この塊で本体をくり抜く」という使い方は現状できません。

**Layer BoundaryがOFFのDividerは、グループを閉じません。** その場合グループは下へ引き継がれ、次のDividerのグループに合流します（入れ子）。レイヤーを分けたいなら、分けたい数だけLayer BoundaryをONにしたDividerが必要です。

**Break Parentは、そこでグループを閉じますがレイヤーにはしません。** 中身は普通にシーンへ混ざります。Group Layoutや複製の単位を整理したいだけのときに使います。
...
Layer Boundaryを使うとStackの読み方が変わります。意図が分かるようにDivider名を変えるのがおすすめです。
...
意図どおりに動いているか確かめたいときは、**2番目のDividerのLayer Blendを0.00と0.50で往復させる**のが一番わかりやすいです。そのグループと、それより前にある形との境目だけが変わり、グループ内部の関係も、後続のレイヤーも動きません。それが確認できれば理解は完成です。
...
Math FieldはLayer Boundaryと非常に相性がよいです。通常のIntersectは過去の形状全体に効いてしまいますが、Math FieldとMask用PrimitiveをひとつのLayerにまとめれば、Math Field Layerだけを切れます。
...
== Layer: Body ==            ← Layer Boundary ON
03 Math Field / Union
04 Cylinder Mask / Intersect
== Layer: Pattern ==         ← Layer Boundary ON
```

**Dividerはグループの下端に置きます**（そのDividerより上がグループ）。本体を `Layer: Body` で閉じ、Math FieldとMaskを次のグループに入れて `Layer: Pattern` で閉じる形です。こうするとCylinder MaskのIntersectはMath Fieldにだけ効き、本体は削れません。
...
| X/Y/Z | ミラーする軸 |
| Offset | 対称面からの距離 |
| Mirror Blend | 継ぎ目を丸める（V16.2.1で追加）。既定 **0.0**。Offsetの2倍を超えると形全体が太り、V16.2.2 から警告が出る |

左右対称パーツや、同じ穴を両側に置きたい場合に使います。

#### Mirror Blend（継ぎ目を丸める）

Mirrorは**空間を折り返す**仕組みで複製しています。そのため2つのコピーは必ず**ハードな最小値**で
接合され、**プリミティブの `Smoothness` はこの継ぎ目に効きません**。折り畳まれた空間の中には
形が1つしか無く、ブレンドする相手が存在しないためです。

これは陰影の問題ではなく、**形そのものの折れ目**でした。半径0.6の球をミラーした実測で、
隣接面の角度が30度を超える辺が44本、最大84.1度。メッシュ全体で鋭い辺はこの44本だけでした。

**`Mirror Blend` を0より大きくすると、折り返さずに両側を別々に評価し、プリミティブ自身の
ブレンド形状（`Profile` / `Chamfer`）で結合します。**

| 設定 | 折れ目の辺 | 最大角 |
|---|---|---|
| Mirror Blend 0.0（従来） | 44本 | 84.1° |
| Mirror Blend 0.3 | **0本** | **8.5°** |
| 実体2つ + Smooth Union（正解） | 0本 | 8.5° |

頂点数まで含めて、実体の球を2つ置いて Smooth Union したものと一致します。近似ではありません。
2軸（X+Z）同時でも折れ目は出ません。

**注意点:**

- **既定は 0.0 です。** そのときは従来と完全に同じ経路を通るので、既存ファイルの見た目は変わりません。
- **プリミティブの `Smoothness`（既定0.2）とその継承は無変更です。** 別の設定なので混同しないでください。
- **評価コストが上がります。** 両側を評価するため、1軸で約1.7倍（res192で31.5ms→53.9ms）。
  軸を増やすと 2の軸数乗 倍になります。
- **ゴーストプレビューは近似表示です（V16.2.2 で改善）。** V16.2.1 では値を上げるとプレビューだけ
  *細く*なり、上げ切ると形が消えていました。**メッシュは当初から正しく**、誤っていたのは表示だけです。
  V16.2.2 でメッシュと同じ向きに膨らむようになり、継ぎ目に折れ目も出なくなりました。
  1軸なら Blend の全域でメッシュとの差は 0.01 程度、継ぎ目そのものは厳密に一致します。
  2軸・3軸を同時に使うと差は広がるので、最終確認は結果メッシュで行ってください。
- **Blend が Offset の2倍を超えると、継ぎ目だけでなく形全体が太ります。** 両側を滑らかに繋ぐ以上
  避けられない挙動で、不具合ではありません。V16.2.2 からその範囲に入るとパネルが警告します。
  形を太らせずに継ぎ目だけ丸めたい場合は、**Blend ではなく Offset を上げてください**。
  なお Mirror Blend の上限は 2.0 なので、Offset が 1.0 以上ならこの範囲には入りません。
- **Radial と Grid は未対応です。** これらも領域繰り返しなので同じ折れ目が出ます
  （実測: Radial 338本/77.2°、Grid 99本/69.9°）。セル境界をまたぐブレンドには
  隣のセルの評価が必要で、ミラーの「2つの側」より一般化が要るためです。
- **Collection Divider 側の Mirror / Radial / Grid には、元からこの問題はありません。**
  そちらはコピーを実体化する経路なので、普通の Smooth Union で繋がります。
...
| Group Layout | Collection Dividerより上のまとまり |
...
2. `Add Collection Divider` を押し、`Layer Boundary` をONにする。これで**本体のグループが閉じる**。
3. その下にMath Fieldを追加する。
4. さらにCylinderを追加し、Intersectにする。
5. もう一度 `Add Collection Divider` を押し、`Layer Boundary` をONにする。これで**装飾のグループが閉じる**。
...
03 Math Field / Union
04 Cylinder Mask / Intersect
== Layer: Gyroid Window ==   ← Layer Boundary ON, Layer Blend 0.00
```

見るべきポイント:

- Cylinder Maskは本体を切らない。Layer内のMath Fieldだけを切る。
- Dividerは**グループの下端**に置く。上に積んだものがそのグループになる。
- 本体側にもDividerが要る。本体をLayerにしないと、装飾を乗せた後のシーンに本体が混ざりにいく。
- `Layer: Gyroid Window` のLayer Blendで、装飾と本体の境目の硬さを調整できる。
...
### Ghost Previewが自動でやっていること（V16.1.1）

Ghost Previewはレイマーチング方式で描画しているため、コストはおおよそ次の式で決まります。

```text
ビューポートのピクセル数 × レイのステップ数 × プリミティブ数
```

SDF.Rは、**操作している最中だけ**品質を落とし、手を止めた瞬間にフル品質へ戻す、という調整を自動で行っています。

| 落としているもの | 操作中の挙動 |
|----|----|
| レイのステップ数 | 視点操作中は64、変形中は128を上限に制限 |
| **描画解像度**（V16.1.1で追加） | 視点操作中は50%、変形中は65%の解像度で描いて拡大 |
| **法線の精度**（V16.1.1で追加） | 6タップの中央差分から3タップの前方差分へ切り替え |

視点操作中にプレビューが少し粗く見えるのはこのためです。**手を止めた瞬間にフル解像度で描き直される**ので、静止画の品質は一切変わりません。生成メッシュにも影響しません。

解像度を落とす方式は**シーンの複雑さに関係なく効く**のが利点で、プリミティブが増えるほど重くなる従来の弱点をある程度カバーします。

### ボトルネックの特定方法

Engine Diagnosticsの `Perf` をONにすると、システムコンソールへ次のログが出ます。
...
| 項目 | 見方 |
|----|----|
| `FPS` | **比較すべきはこちら。** 実際の描画頻度 |
| `avg duration` | **改善の指標にならない。** これはCPU側が描画命令を発行して戻るまでの時間で、GPUは非同期に動くためレイマーチのコストを含まない |
| `texture rebuilds` | **多い場合はPython側のプレビュー用バッファ再構築がボトルネック。** 描画ではないため、上記の解像度調整では改善しない |

典型的な切り分け:

- 静止時も視点操作時もFPSが高い → 描画は律速ではない。
- 視点操作でFPSが落ちる、`texture rebuilds` は0 → 描画が律速。Resolutionやプリミティブ数を見直す。
- オブジェクトを動かした時だけFPSが落ち、`texture rebuilds` が多い → 更新処理が律速。Mesh iconをOFFにする、Live UpdateをOFFにして手動更新にする、などが効く。
...
Layer Boundaryを使います。切りたい装飾やMath Fieldと、Intersect用Primitiveを続けて積み、**そのすぐ下**にLayer BoundaryをONにしたDividerを置いて閉じます（Dividerより上がグループです）。本体側も別のDividerでLayerにしておかないと、本体が後から混ざりにいくので注意してください。
...
### Q. 視点を回している間、プレビューが少しぼやける

V16.1.1からの仕様です。操作中だけ解像度を落として描画し、手を止めた瞬間にフル解像度で描き直します。重いシーンでの視点操作を軽くするための処理で、静止画の品質や生成メッシュには影響しません。詳細は「20. パフォーマンス調整」を参照してください。

### Q. ゴーストプレビューが出なくなった

**V16.2.0 の不具合です。V16.2.1 で修正済みですので、更新してください。**

1本のツリーの要素が17個以上になると、プレビュー用テクスチャの生成に失敗し、ゴースト表示だけが
止まっていました。**数えるのはレイアウト展開後の要素数**なので、仕切り（Divider）の Grid や Radial を
効かせている場合は、配置した部品が少なくても該当します（部品3個でも 4×4 の配列で18要素になります）。

**メッシュ生成には影響しません。** 計算は別経路のため、出力メッシュも保存済みのファイルも正しいまま
です。更新すればプレビューは元に戻り、その間に保存したファイルでも問題なく表示されます。

V16.2.1 以降でプレビューが出ない場合は、次を確認してください。

- ヘッダーの **Show GPU Preview**（ゴーストのアイコン）と **Live Update** がONか
- **編集したいツリーがアクティブになっているか**。プレビューはアクティブなツリー1本だけを描きます
  （結果メッシュのほうは全ツリーぶん常に表示されます）
...
| `sdf.add_tree` | Add SDF Tree | 新しいSDFツリーを追加（V16.2.1） |
| `sdf.set_active_tree` | Set Active SDF Tree | 編集対象のツリーを切り替え（V16.2.1） |
| `sdf.add_tree_ref` | Add Tree Reference | 別ツリーを取り込む参照Emptyを追加（V16.2.1） |
...
| Layer Boundary | Dividerより上のグループを独立したLayerとして評価する境界 |
...
SDF.R V16.2.2の基本は、PrimitiveをStackへ積み、OperationとSmoothnessで形を作ることです。そこにMath Field、Layer Boundary、Layout、Deformを加えることで、単なるブーリアンツールを超えたSDFモデリング環境になります。

V16.1.0では、これに「手で描く」という選択肢が加わりました。数値で組み立てるのが向く形はPrimitiveで、描いたほうが早い形はCurve Syncで、と使い分けられます。V16.1.1では、その環境を快適に扱えるようビューポートの応答性が改善されています。
