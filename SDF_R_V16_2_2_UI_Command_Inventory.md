# SDF.R V16.2.2 UI / Command Inventory

対象: **Rust-GPU-SDF / SDF.R V16.2.2** \| 作成日: 2026-07-19（V16.2.1 追記: 2026-09-09 / V16.2.2 追記: 2026-09-12） \| 用途: UIボタン・主要コントロールの棚卸し資料
...
パネル最上段。**ツリーが1本のときも常に表示されます。**

| 表示名 | 種別 | 内容 | 再計算 | 補足 |
|----|----|----|----|----|
| Tree | ドロップダウン `sdf_scene_props.active_output` | 編集対象のSDFツリーを選ぶ。以下のパネル全体（Parts / The Stack / Output & Quality）が選択に追従する。 | 表示/管理 | ビューポートでパーツを選ぶだけでも、そのパーツが属するツリーへ切り替わる。 |
| ＋ | ボタン `sdf.add_tree` | 新しいSDFツリーを追加する。既存ツリーは退避されない。 | Mesh再計算 | 1本目は `SDF_Collection` / `SDF_Result` のまま。2本目以降が連番。 |
| Parts: | 状態表示 | 現在のツリーのパーツ用コレクション名。 | 表示/管理 | — |
| N trees | 状態表示 | シーン内のツリー本数。2本以上のときだけ表示。 | 表示/管理 | — |
...
| Ghost icon | トグル | GPU Ghost Previewの表示を切り替える。 | 表示/管理 | メッシュ化前のリアルタイムプレビュー用。**V16.2.2: Mirror Blend の描画が最終メッシュと逆向きだったのを修正**（メッシュ側は当初から正しい。折れ目も出なくなった）。**V16.2.1: 1本のツリーの要素が17個以上のときプレビューが出なくなる不具合を修正**（V16.2.0 の退行。数えるのはレイアウト展開後の要素数）。 |
...
| Operationアイコン | 選択 | Union / Subtract / Intersect を切り替える。 | Mesh再計算 | 形状評価の根幹。Layer内ではそのグループにだけ作用。 |
...
| Layer Boundary icon | トグル | そのDividerより上のグループをローカルLayerとして評価し、結果を全体へUnionする。 | Mesh再計算 | 分けたい部品ごとにDividerを立てる。 |
...
| Finalize (Bake) | ボタン `sdf.finalize` | SDF結果を通常メッシュとして確定し、使用プリミティブを履歴へ退避する。 | Mesh再計算/確定 | Live編集を終える操作。戻すなら履歴や前バージョンを使う。**V16.2.1: 対象はアクティブなツリー1本のみ。**他のツリーが残っている間は Live Update も切れず、編集を続けられる。最後の1本を確定したときだけ従来どおり Live Update が切れる。 |
...
| Layer Boundary | トグル | Dividerより上のグループをローカルLayerとして評価する。 | Mesh再計算 | `Base ∪ (Layer内評価結果)` のイメージ。合流の強さは Layer Blend。 |
| Layer Blend | 数値 | Layerをシーンへ合流させるときのブレンド半径。既定0.0（硬い境界）。 | Mesh再計算 | Layer BoundaryがONのときだけ表示。V16.2.0で追加。 |
| Profile (Layer) | 選択 | 合流時のブレンド形状。Round / Sharp / Soft / Tight / Chamfer。 | Mesh再計算 | Layer BoundaryがONのときだけ表示。V16.2.0で追加。 |
...
## 8-B. Tree Reference（V16.2.1 で追加）

`SDF_TreeRef_NNN` という名前のEmptyを選択したときに出る。シーンに複数ツリーがあるときは追加ボタンが出る。

| 表示名 | 種別 | 内容 | 再計算 | 補足 |
|----|----|----|----|----|
| Add Tree Reference | ボタン `sdf.add_tree_ref` | 別ツリーを取り込むための参照Emptyをスタックに追加する。 | Mesh再計算 | 作成時は参照先の上に置かれる。動かさなければ位置が一致する。 |
| Tree | 参照 `tree_ref_obj` | 取り込む相手のツリー（出力オブジェクト）を選ぶ。 | Mesh再計算 | 自己参照と相互参照は無視される（無限ループ防止）。 |
| Mode | 列挙 `tree_ref_mode` | `Blend`（結合）/ `Subtract`（減算）。 | Mesh再計算 | **Blendは厳密。Subtractは参照先がUnionのみなら厳密**、内部でSubtract/Intersectを使っていると近似。 |
| Smoothness | 数値 | 取り込み時のブレンド半径。 | Mesh再計算 | — |
| Profile / Chamfer | 列挙 / 数値 | Blend時のブレンド形状。 | Mesh再計算 | Modeが`Blend`のときのみ表示。 |
| （注意表示） | 情報 | 参照先が内部でSubtract/Intersectを使っている場合に表示される。 | 表示/管理 | Modeが`Subtract`のときのみ。 |

> **参照の入れ子は1段まで。** 参照先がさらに別ツリーを参照していても、そこは辿らない。
> 参照先を編集すると、取り込み側も自動で更新される（状態ハッシュに畳み込み済み）。
...
| Mirror Blend | 数値 `mirror_blend` （V16.2.1） | ミラーの継ぎ目を丸める。**既定 0.0**。**V16.2.2: Offsetの2倍を超えると形全体が太る旨の警告がパネルに出る**（上限2.0のため Offset 1.0以上では到達しない）。 | Mesh再計算 | **0のときは従来と完全に同じ経路**。>0で両側を別々に評価するため、評価コストが1軸あたり約1.7倍、軸を増やすごとに倍。プリミティブの `Smoothness` はこの継ぎ目に効かないため、丸めたいときはこちらを使う。**Radial / Grid は未対応**。プレビューは近似表示。 |
...
Source basis: `rust_gpu_sdf_addon/ui.py`, `operators.py`, `properties.py`, `constants.py` in Rust-GPU-SDF-V16.2.2.
