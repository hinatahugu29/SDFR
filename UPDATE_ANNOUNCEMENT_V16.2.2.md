...
1. `SDF.R V16.2.2 — one add-on, separate objects`
2. `SDF.R V16.2.2 is out (free update) — a scene can now hold several SDF trees`
3. `[SDF.R] V16.2.2 released — the parts of your model can be separate meshes now`
4. `SDF.R V16.2.2 — separate objects, and a ghost preview fix for V16.2.0 users`
...
**Japanese subject:** `SDF.R V16.2.2 公開 — パーツを別オブジェクトに / V16.2.0 のプレビュー不具合を修正`

*V16.2.1 は公開していません。* リリース直前に Mirror Blend のプレビュー不具合が見つかり、その修正版が
V16.2.2 になりました。**番号が飛ぶ理由を1行添えておくと、問い合わせが減ります。** 件名に入れる必要は
ありません（本文の冒頭で足ります）。
...
**V16.2.2 is now available as a free update for all owners.**
...
*You did not miss V16.2.1.* It was built and tested but never published — a fault turned up in the
Mirror Blend preview before release, and fixing it produced this build instead. Everything V16.2.1
would have contained is here.
...
Two honest limits. The ghost preview shows this as an approximation — it swells in the same
direction as the mesh and tracks it closely, but the two do not round identically. And **Radial and
Grid still crease**: blending across cell boundaries needs the neighbouring cells, which is a bigger
change.

One thing worth knowing before you reach for it: **raising Blend past twice the Offset thickens the
whole shape, not just the seam.** That is inherent to joining the two halves smoothly, and the panel
now warns you when your settings cross that line. If you want a rounder seam without a heavier shape,
raise Offset rather than Blend.
...
**V16.2.2 を公開しました。既存ユーザーの方は無償アップデートです。**
...
*V16.2.1 は公開していません。* ビルドとテストまでは済んでいましたが、リリース直前に Mirror Blend の
プレビュー不具合が見つかり、その修正版が V16.2.2 になりました。**V16.2.1 に入る予定だった内容は
すべてこのバージョンに入っています。**
...
正直に2点。ゴーストプレビューは近似表示です（メッシュと同じ向きに膨らみ、よく追従しますが、
丸まり方が完全に一致するわけではありません）。また **Radial と Grid には引き続き折れ目が出ます** —
セル境界をまたぐブレンドには隣のセルの評価が必要で、ミラーより大きな変更になるためです。

使う前に1点だけ。**Blend を Offset の2倍より大きくすると、継ぎ目だけでなく形全体が太ります。**
両側を滑らかに繋ぐ以上、避けられない挙動で、その範囲に入るとパネルが警告を出します。形を太らせずに
継ぎ目だけ丸めたい場合は、Blend ではなく Offset を上げてください。
