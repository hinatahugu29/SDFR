...
| V16.1.3 検証時 | **報告された条件（Boolean との組み合わせ）を再現しないまま「修正完了」としていた** | 購入者の報告原文は "Symmetry doesn't seem to function correctly with **Booleans**"。しかし検証は Union 単体のプリミティブのみで、Subtract / Intersect を一度も試していなかった | **報告文の条件をそのまま再現するテストを書く。** `scratch/test_boolean_equivalence.py` で確認したところ、V16.1.2 では負側の Subtract が**丸ごと無視され、切り欠きのない土台がそのまま出力**されていた（頂点数が「箱のみ」と完全一致）。単体形状なら空メッシュで気づくが、Boolean だと土台が正常に出るぶん「プレビューには出るのに最終メッシュに出ない」という見え方になる |
| V16.1.3 検証時 | Boolean 用に書いた最初のテストが、**壊れている V16.1.2 を OK と判定した** | 判定指標を「出力メッシュが左右対称か」にしていた。切り欠きが無視された箱は、切り欠きが無いぶん**完全に左右対称**になる | 対称性ではなく**等価性**で判定する。「切り欠きを X=+2 に置いた結果」と「X=−2 に置いた結果」が一致するか。対称化が正しければ両者は同一になるはずで、メッシュ化由来の微小な非対称（切り欠き無しの箱でも 1.7% 出る）にも影響されない |
